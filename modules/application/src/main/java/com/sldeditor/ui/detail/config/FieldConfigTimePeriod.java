...
public class FieldConfigTimePeriod extends FieldConfigBase implements UndoActionInterface {
    ...
    private void createUIPanel(
        ...
        panelData.dateModel = new DatePickerSettings();
        panelData.timeModel = new TimePickerSettings();
        panelData.dateModel.setAllowEmptyDates(false);
        panelData.timeModel.setAllowEmptyTimes(false);
        panelData.timeModel.setDisplaySpinnerButtons(true);
        panelData.timeModel.setDisplayToggleTimeMenuButton(false);
        panelData.timeModel.setFormatForDisplayTime(
                PickerUtilities.createFormatterFromPatternString(
                        "HH:mm:ss.SSS", panelData.timeModel.getLocale()));

        panelData.dateTimePicker = new DateTimePicker(panelData.dateModel, panelData.timeModel);

        int xPos = getXPos();
        panelData.dateTimePicker.setBounds(
                xPos + BasePanel.WIDGET_X_START,
                getRowY(row),
                BasePanel.WIDGET_STANDARD_WIDTH * 2,
                BasePanel.WIDGET_HEIGHT);
        panelData.panel.add(panelData.dateTimePicker);

        panelData.dateTimePicker.addDateTimeChangeListener(
    ...
    public void internal_setEnabled(boolean enabled) {
        if (start.dateTimePicker != null) {
            start.dateTimePicker.setEnabled(enabled);
        }
    }
    ...
    public boolean isEnabled() {
        if ((attributeSelectionPanel != null) && !isValueOnly()) {
            ...
            if (start.dateTimePicker != null) {
                return start.dateTimePicker.isEnabled();
            }
        }
        return false;
    }
     ...
     * @return the date
     */
    private ZonedDateTime getDate(TimePeriodPanel panel) {
        if (panel == null) {
            return null;
        }
        ...
        ZonedDateTime zonedDateTime = panel.dateTimePicker.getDateTimeStrict().atZone(panel.zoneId);
            ...
            if (timePeriodPanel.areFieldsConfigured()) {
                if (timePeriodPanel.dateModel != null) {
                    if (duration.isDate()) {
                        ...
                        timePeriodPanel.dateTimePicker.setDateTimeStrict(
                                duration.getDate().toLocalDateTime());
                        timePeriodPanel.zoneId = duration.getDate().getZone();
                    } else {
    ...
    public void setVisible(boolean visible) {
        if (start.dateTimePicker != null) {
            start.dateTimePicker.setVisible(visible);
        }
        if (end.dateTimePicker != null) {
            end.dateTimePicker.setVisible(visible);
        }
    }
    ...
    private void updateFields(TimePeriodPanel panel) {
        ...
        if (panel.areFieldsConfigured()) {
            ...
            panel.dateTimePicker.setEnabled(dateEnabled);
