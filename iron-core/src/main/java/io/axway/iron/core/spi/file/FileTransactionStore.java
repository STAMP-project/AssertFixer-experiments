...
public class FileTransactionStore implements TransactionStore {
    ...
    String getFileName(long id, String storeName) {
    ...
    public void seekTransaction(BigInteger latestProcessedTransactionId) {
        checkThatLastTransactionIdIsAlignedWithLastSnapshotId(latestProcessedTransactionId);
    ...
    /**
     * Check that there is a transaction corresponding to the latestProcessedTransactionId.
     * If that is not the case, new transactions are created with a transactionId lower thant the snapshotId.
     * When restarting on the snapshot, these transaction would not be processed.
     *
     * @param latestProcessedTransactionId the latestProcessedTransactionId
     */
    private void checkThatLastTransactionIdIsAlignedWithLastSnapshotId(BigInteger latestProcessedTransactionId) {
        String expectedTransactionStartingName = Strings.padStart(latestProcessedTransactionId.toString(), 20, '0');
        Pattern expectedTransactionPattern = Pattern.compile(expectedTransactionStartingName + "_.+\\.tx");
        try (Stream<Path> pathStream = Files.find(m_transactionDir, 1, (path, atts) -> {
            if (atts.isDirectory()) {
                return false;
            }
            String fileName = path.getFileName().toString();
            return expectedTransactionPattern.matcher(fileName).matches();
        })) {
            long count = pathStream.count();
            if (count != 1) {
                throw new UnrecoverableStoreException(
                        "Iron store is corrupted." + " Expected to find a single transaction file in " + m_transactionDir + " that matches the pattern "
                                + expectedTransactionPattern.pattern() + ", but found " + count + " transaction files.");
            }
        } catch (IOException e) {
            throw new UnrecoverableStoreException("Cannot succeed to open the Iron File Transaction Store",
                                                  args -> args.add("transactionDir", m_transactionDir), e);
        }
    }
