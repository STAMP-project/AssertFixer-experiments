...
public final class CassandraProcess
	...
	protected void onAfterProcessStart(ProcessControl process,
		...
		new ReaderProcessor(process.getReader(), StreamToLineProcessor.wrap(
				new ProcessIdStreamProcessor(getProcessId(), logWatch))).start();
		new ReaderProcessor(process.getError(), StreamToLineProcessor.wrap(
				new ProcessIdStreamProcessor(getProcessId(), processOutput.getError()))).start();

		logWatch.waitForResult(executableConfig.getTimeout().toMillis());
		String failureFound = logWatch.getFailureFound();
		if (!logWatch.isInitWithSuccess()) {
			String msg = "Could not start a process '" + getProcessId() + "'.\nFailure:" + failureFound +
					"\nOutput:\n--- START --- \n" + logWatch.getOutput() + "\n--- END --- ";
			throw new IOException(msg);
		}
		checkTransport(config);
		log.info("Cassandra process '{}' has been started.", getProcessId());
	...
	private void checkTransport(Config config) throws IOException {
		...
		if (ClientConnection.isEnabled(config) && !ClientConnection.isConnected(config, maxAttempts, sleep)) {
			throw new IOException(
					"Could not start a process '" + getProcessId() + "'. Something wrong with a client transport.");
		}
	}
	...
	private static abstract class ProcessStopper {
		...
		private static void killProcess(Context context, long pid) {
			...
			ExecutableConfig executableConfig = context.getExecutableConfig();
			IStreamProcessor output = StreamToLineProcessor.wrap(
					new ProcessIdStreamProcessor(pid, runtimeConfig.getProcessOutput().getCommands()));
	...
	/**
	 * Utility class for appending process id to each log event.
	 */
	private static class ProcessIdStreamProcessor implements IStreamProcessor {

		private final long processId;

		private final IStreamProcessor delegate;

		ProcessIdStreamProcessor(long processId, IStreamProcessor delegate) {
		...
		@Override
		public void process(String block) {
			this.delegate.process("[P:" + this.processId + "] " + block);
		}
	 ...
	 */
	private static class ReaderProcessor extends Thread {

		private final Reader reader;

		private final IStreamProcessor processor;

		ReaderProcessor(Reader reader, IStreamProcessor processor) {
			this.reader = reader;
		...
		public void run() {
			try {
				...
				while ((read = this.reader.read(buf)) != -1) {
					this.processor.process(new String(buf, 0, read));
				}
			}
			catch (IOException ex) {
				log.error("Process I/O error occurs", ex);
			}
			finally {
