...
public class CommandTest
{
	...
	{
		CommandLineParser parser = CommandLineParser.withCommands(new Git(new Repository())).andArguments(Git.MESSAGE);
		parser.parse("git", "commit", "--amend", "--message", "a", "--author=a");
	}
	...
	{
		Argument<String> likesP = stringArgument("-p").limitTo(s -> s.contains("p")).defaultValue("pppp").build();
		...
		{
			assertThat(expected.getMessageAndUsage()).isEqualTo("");
		}
	}
