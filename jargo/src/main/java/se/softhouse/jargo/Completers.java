...
final class Completers
{
	...
	private static final class BashCompleter implements Completer
	{
		...
		SortedSet<String> complete(CommandLineParserInstance parser, Iterable<String> args, String partOfWord)
		{
			...
			ParsedArguments holder = new ParsedArguments(parser.allArguments(), parser);
			...
			ArgumentIterator iterator = ArgumentIterator.forArguments(args, emptyMap(), handler);
				...
				ArgumentIterator iterator)
		{
			...
			SortedSet<String> matchingPrefix = StringsUtil.prefixes(partOfWord, options);
