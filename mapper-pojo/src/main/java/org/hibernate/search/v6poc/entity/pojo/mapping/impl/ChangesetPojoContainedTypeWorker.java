	...
	private class WorkPlanPerDocument {
		...
		void add(Supplier<E> entitySupplier) {
			...
			if ( createdInThisChangeset == null ) {
				// No update yet, so we actually did createExtractingMatcher the entity in this changeset
		...
		void delete(Supplier<E> entitySupplier) {
			this.entitySupplier = entitySupplier;
			if ( createdInThisChangeset == null ) {
				// No add or update yet, and we're performing a delete, so we did not createExtractingMatcher the entity in this changeset
		...
		private void doUpdate(Supplier<E> entitySupplier) {
			this.entitySupplier = entitySupplier;
			if ( createdInThisChangeset == null ) {
				// No add yet, and we're performing an update, so we did not createExtractingMatcher the entity in this changeset
				createdInThisChangeset = false;
			}
		}
