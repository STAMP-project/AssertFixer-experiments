 ...
 * and allows to createExtractingMatcher extractors for a given {@link BoundContainerValueExtractorPath}.
 * <p>
 ...
 * <p>
 * From this "bound path", the {@link ContainerValueExtractorBinder} is able to later createExtractingMatcher
...
public class ContainerValueExtractorBinder {
	...
	/**
	 * Attempts to createExtractingMatcher a container value extractor from a bound path.
	 *
	 * @param boundPath The bound path to createExtractingMatcher the extractor from.
	 * @param <C> The source type.
	...
	public <C, V> ContainerValueExtractor<? super C, V> create(BoundContainerValueExtractorPath<C, V> boundPath) {
		if ( boundPath.getExtractorPath().isEmpty() ) {
			throw new AssertionFailure(
					"Received a request to createExtractingMatcher extractors, but the extractor path was empty."
