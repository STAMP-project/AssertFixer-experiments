...
public class Execution implements AccessExecution, Archiveable<ArchivedExecution>, LogicalSlot.Payload {
			...
			releaseFuture.whenComplete(
				(Object ignored, Throwable throwable) -> {
					if (logicalSlotFuture.cancel(false)) {
						LOG.info("Canceled slot request with slot request id [{}].", slotRequestId);
						slotProvider.cancelSlotRequest(
							slotRequestId,
							slotSharingGroupId,
							new FlinkException("Execution " + this + " was released."));
					} else {
						LOG.info("Failed to cancel logical slot future for slot request id [{}]. Slot future is done {}, completed exceptionally {}.", slotRequestId, logicalSlotFuture.isDone(), logicalSlotFuture.isCompletedExceptionally());
					}
				});

			return logicalSlotFuture.thenApply(
				(LogicalSlot logicalSlot) -> {
					LOG.info("Assign logical slot [{}] to slot request id [{}].", logicalSlot.getAllocationId(), slotRequestId);
					if (tryAssignResource(logicalSlot)) {
						LOG.info("Assigned slot [{}] to execution {} with slot request id [{}].", logicalSlot.getAllocationId(), this, slotRequestId);
						return this;
					} else {
						LOG.info("Could not assign slot [{}] to execution {} with slot request id [{}].", logicalSlot.getAllocationId(), this, slotRequestId);
						// release the slot
	...
	private void releaseAssignedResource(@Nullable Throwable cause) {
		...
		if (slot != null) {
			LOG.info("Release assigned resource slot [{}] for execution {} with slot request id [{}].", slot.getAllocationId(), this, slot.getSlotRequestId());
			...
			LOG.info("No assigned resource to release for execution {}.", this);
