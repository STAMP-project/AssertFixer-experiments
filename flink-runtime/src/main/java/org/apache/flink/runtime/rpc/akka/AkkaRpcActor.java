			...
			try {
				...
				if (rpcMethod.getReturnType().equals(Void.TYPE)) {
					...
					if (result instanceof CompletableFuture) {
						...
						Method finalRpcMethod = rpcMethod;
						future.whenComplete(
							(value, throwable) -> {
								if (finalRpcMethod.getName().equalsIgnoreCase("allocateSlot")) {
									try {
										log.info("Completed allocate slot for [{}] with value {}.", rpcInvocation.getArgs()[0], value);
									} catch (IOException e) {
										e.printStackTrace();
									} catch (ClassNotFoundException e) {
										e.printStackTrace();
