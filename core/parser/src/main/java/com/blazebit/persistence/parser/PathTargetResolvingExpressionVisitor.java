...
public class PathTargetResolvingExpressionVisitor implements Expression.Visitor {
    ...
    private Type<?> getType(Type<?> baseType, Attribute<?, ?> attribute) {
        ...
        // Workaround HHH-12969: Hibernate returning the member of the identifier attribute of
        // the IdClass as member of the association attribute on the entity.
        if (attribute instanceof SingularAttribute) {
            if (member instanceof Method) {
                member = ReflectionUtils.getMethod(baseClass, member.getName());
            } else if (member instanceof Field) {
                member = ReflectionUtils.getField(baseClass, member.getName());
            }
            if (member == null && attribute.getJavaMember() != null) {
                throw new IllegalStateException("Cannot resolve " + attribute.getJavaMember().getName() + " on " + baseClass.getSimpleName());
            }
        }

        if (baseClass != null) {
            if (attribute.getJavaMember() instanceof Field) {
                return metamodel.type(ReflectionUtils.getResolvedFieldType(baseClass, (Field) member));
            } else if (attribute.getJavaMember() instanceof Method) {
                return metamodel.type(ReflectionUtils.getResolvedMethodReturnType(baseClass, (Method) member));
            }
        }
