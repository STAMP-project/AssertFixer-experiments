...
public abstract class AbstractFullQueryBuilder<T, X extends FullQueryBuilder<T, X>, Z, W, FinalSetReturn extends BaseFinalSetOperationBuilderImpl<T, ?, ?>> extends AbstractQueryBuilder<T, X, Z, W, FinalSetReturn> implements FullQueryBuilder<T, X> {
    ...
    protected final String buildPageCountQueryString(StringBuilder sbSelectFrom, boolean externalRepresentation) {
        if (!havingManager.isEmpty()) {
            throw new IllegalStateException("Count queries when HAVING predicates are present are not yet supported!");
        }
        ...
        boolean isResultUnique = appendIdentifierExpressions(sbSelectFrom);

        sbSelectFrom.append(")");
        int countEndIdx = sbSelectFrom.length() - 1;
