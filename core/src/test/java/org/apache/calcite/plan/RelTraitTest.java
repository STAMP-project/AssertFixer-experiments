...
public class RelTraitTest {
  ...
  private void assertCanonical(String message, Supplier<List<RelCollation>> collation) {
    RelTrait emptyCollation =
        RelCompositeTrait.of(COLLATION, collation.get());
    RelTrait emptyCollation2 =
        RelCompositeTrait.of(COLLATION, collation.get());

    Assert.assertEquals(
        "RelCompositeTrait.of should return the same instance for " + message,
        emptyCollation + " " + System.identityHashCode(emptyCollation),
        emptyCollation2 + " " + System.identityHashCode(emptyCollation2));
  }
