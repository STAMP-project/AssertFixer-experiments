...
/**
 * {@link Configuration Configuration} for {@link com.github.nosan.embedded.cassandra.spring.EmbeddedCassandra Embedded
 * Cassandra}
...
class EmbeddedCassandraConfiguration {
	...
	private static final Logger log = LoggerFactory
			.getLogger(EmbeddedCassandraConfiguration.class);
	...
	private static class EmbeddedClusterBeanFactoryPostProcessor
		...
		private BeanDefinitionHolder getClusterBeanDefinition(
			...
			log.info(
					"There is no Cluster beans. Embedded primary '{}' Cluster bean will be registered",
					BEAN_NAME);
	...
	private static class EmbeddedClusterFactoryBean
		...
		EmbeddedClusterFactoryBean(
				ObjectProvider<BootstrapSettings> bootstrapSettings,
				ApplicationContext applicationContext) {
			this.context = applicationContext;
			this.cassandra = new EmbeddedCassandra(bootstrapSettings.getIfAvailable());
		}
		...
		public void afterPropertiesSet() throws Exception {
			...
			CqlScriptUtils
					.execute(this.cassandra.getSession(), getCqlScripts(this.context));
		}
		...
		private static CqlScript[] getCqlScripts(ApplicationContext context)
				throws IOException {
			Environment env = context.getEnvironment();
			String[] scripts = env.getProperty(SCRIPTS, String[].class, new String[0]);
			String[] statements = env
					.getProperty(STATEMENTS, String[].class, new String[0]);
			Class<?> testClass = env.getProperty(TEST_CLASS, Class.class);
			String encoding = env.getProperty(ENCODING);
			return SpringCqlUtils.getScripts(context,
					new CqlConfig(testClass, scripts, statements, encoding));
