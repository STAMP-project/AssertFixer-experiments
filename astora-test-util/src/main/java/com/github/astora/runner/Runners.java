...
public final class Runners {
    ...
    public static <T extends Runner & Filterable> SingleTestResult runSingleTest(
            T runner, String name) {
        try {
    ...
    private static final <T extends Runner & Filterable> SingleTestResult runSingleTestInternal(
            T runner, String name) throws ClassNotFoundException, NoTestsRemainException {
        final TestStatus[] successful = {TestStatus.SUCCESS};
        ...
        notifier.addListener(new RunListener() {
            ...
            public void testFailure(Failure failure) throws Exception {
                successful[0] = TestStatus.FAILURE;
                exception[0] = failure.getException();
            }

            @Override
            public void testAssumptionFailure(Failure failure) {
                successful[0] = TestStatus.ASSUMPTION_FAILURE;
                exception[0] = failure.getException();
            }

            @Override
            public void testIgnored(Description description) throws Exception {
                successful[0] = TestStatus.IGNORED;
            }
        });
