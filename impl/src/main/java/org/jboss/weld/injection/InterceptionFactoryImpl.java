...
public class InterceptionFactoryImpl<T> implements InterceptionFactory<T> {
                ...
                for (Annotation annotation : annMethod.getAnnotations()) {
                    if (isInterceptorBinding(annotation)) {
                        newAtConfigurator.filterMethods((m) ->
                            ...
                            isMatchingMethod(m.getJavaMember(), annMethod.getJavaMember())
                            ).findFirst().get().add(annotation);
                    }
                }
            }
    ...
    private boolean isInterceptorBinding(Annotation annotation) {
        return annotation.annotationType().getAnnotation(InterceptorBinding.class) != null;
    }

    private boolean isMatchingMethod(Method first, Method second) {
        ...
        List<Class<?>> secondMethodParams = Arrays.asList(second.getParameterTypes());
        for (Class<?> paramType : first.getParameterTypes()) {
            if (!secondMethodParams.contains(paramType)) {
                return false;
            }
        }
        return true;
