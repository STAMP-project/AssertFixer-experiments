...
public abstract class AbstractJournalUpdateTask implements JournalReaderCallback {
   ...
   private ByteBuffer writingChannelNioBuffer;
         ...
         try {
            if (sequentialFile.isOpen()) {
               try {
                  ...
                  sequentialFile.writeDirectAndSync(byteBuffer, false);
               } finally {
            ...
            if (releaseWritingBuffer) {
               ...
               fileFactory.releaseDirectBuffer(writingChannelNioBuffer);
               writingChannel = null;
               writingChannelNioBuffer = null;
            }
         }
      }
   }
   ...
   protected void openFile() throws Exception {
      ...
      if (writingChannelNioBuffer != null && writingChannelNioBuffer.capacity() < fileSize) {
         fileFactory.releaseDirectBuffer(writingChannelNioBuffer);
         writingChannelNioBuffer = null;
         writingChannel = null;
      }
      if (writingChannelNioBuffer == null) {
         final ByteBuffer bufferWrite = fileFactory.allocateDirectBuffer(fileSize);
         writingChannelNioBuffer = bufferWrite;
         writingChannel = ActiveMQBuffers.wrappedBuffer(bufferWrite);
      } else {
         writingChannel.clear();
         writingChannelNioBuffer.clear();
      }
