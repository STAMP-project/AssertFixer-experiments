...
public class TransformSpatialPredicates
{
    ...
    public TransformSpatialPredicates(Metadata metadata, SplitManager splitManager, PageSourceManager pageSourceManager)
    {
    ...
    public Set<Rule<?>> rules()
    {
        return ImmutableSet.of(
                new TransformSpatialPredicateToJoin(metadata, splitManager, pageSourceManager),
                new TransformSpatialPredicateToLeftJoin(metadata, splitManager, pageSourceManager));
    }
            ...
            implements Rule<FilterNode>
    {
        ...
        public TransformSpatialPredicateToJoin(Metadata metadata, SplitManager splitManager, PageSourceManager pageSourceManager)
        {
        ...
        {
            String kdbTree = loadKdbTree("kdb_tree", context.getSession(), metadata, splitManager, pageSourceManager);
            System.out.println(kdbTree);
            ...
            implements Rule<JoinNode>
    {
        ...
        public TransformSpatialPredicateToLeftJoin(Metadata metadata, SplitManager splitManager, PageSourceManager pageSourceManager)
        {
            this.metadata = metadata;
            this.splitManager = splitManager;
            this.pageSourceManager = pageSourceManager;
    ...
    private static String loadKdbTree(String tableName, Session session, Metadata metadata, SplitManager splitManager, PageSourceManager pageSourceManager)
    {
        ...
        TableHandle tableHandle = metadata.getTableHandle(session, name)
                .orElseThrow(() -> new IllegalArgumentException(format("Table %s does not exist", name)));
        Map<String, ColumnHandle> columnHandles = metadata.getColumnHandles(session, tableHandle);
        if (columnHandles.size() != 1) {
            throw new IllegalArgumentException(format("Expected single column for table %s, but found %s columns", name, columnHandles.size()));
        }

        ColumnHandle columnHandle = Iterables.getOnlyElement(columnHandles.values());

        List<TableLayoutResult> layouts = metadata.getLayouts(session, tableHandle, new Constraint<>(TupleDomain.all(), bindings -> true), Optional.of(ImmutableSet.of(columnHandle)));
        if (layouts.isEmpty()) {
            throw new IllegalArgumentException(format("Got no layout for table %s", name));
        }
        SplitSource splitSource = splitManager.getSplits(session, layouts.get(0).getLayout().getHandle(), UNGROUPED_SCHEDULING);
        ...
        while (true) {
            ...
            for (Split split : splits) {
                ConnectorPageSource pageSource = pageSourceManager.createPageSource(session, split, ImmutableList.of(columnHandle));
                getFutureValue(pageSource.isBlocked());
                Page page = pageSource.getNextPage();
                if (page != null) {
                    if (page.getPositionCount() != 1) {
                        throw new IllegalArgumentException(format("Expected exactly one row for table %s, but found %s rows", name, page.getPositionCount()));
                    }
        ...
        throw new IllegalArgumentException(format("Expected exactly one row for table %s, but got none", name));
    }
