...
public class CompilerErrorsTest {
    ...
    public void testNavigateOnSameLevelAsSteps() throws Exception {
        ...
        exception.expect(RuntimeException.class);
        exception.expectMessage("Error compiling source 'flow_navigate_same_level_as_step'.\n" + "Flow: 'flow_navigate_same_level_as_step' has steps with keyword on the same indentation as the step name " + "or there is no space between step name and hyphen.");
        compiler.compile(SlangSource.fromFile(resource), path);
    }
    ...
    public void testNavigateToNonExistingStep() throws Exception {
        ...
        path.add(SlangSource.fromFile(operation));
        exception.expect(RuntimeException.class);
        exception.expectMessage("Failed to compile step: Step1. The step/result name: non_existing_step of navigation: " + "SUCCESS -> non_existing_step is missing");
        compiler.compile(SlangSource.fromFile(resource), path);
    }
    ...
    public void testNotOpFlowFile() throws Exception {
        ...
        Set<SlangSource> path = new HashSet<>();
        exception.expect(RuntimeException.class);
        exception.expectMessage("Error transforming source: no_op_flow_file to a Slang model. " + "Source no_op_flow_file has no content associated with flow/operation/decision/properties property.");
        compiler.compile(SlangSource.fromFile(resource), path);
    }
    ...
    public void testFlowWithNavigationToMissingStep() throws Exception {
        ...
        exception.expect(RuntimeException.class);
        exception.expectMessage("Failed to compile step: Step1. The step/result name: " + "Step2 of navigation: SUCCESS -> Step2 is missing");
        compiler.compile(SlangSource.fromFile(resource), path);
    }
    ...
    public void testFlowWithNavigationToMissingDefaultResults() throws Exception {
        ...
        path.add(SlangSource.fromFile(operations));
        exception.expect(RuntimeException.class);
        exception.expectMessage("Failed to compile step: Step1. " + "The step/result name: SUCCESS of navigation: SUCCESS -> SUCCESS is missing");
        compiler.compile(SlangSource.fromFile(resource), path);
    }
    ...
    public void testFlowWithMissingNavigationFromOperationResult() throws Exception {
        ...
        path.add(SlangSource.fromFile(operations));
        exception.expect(RuntimeException.class);
        exception.expectMessage("Cannot compile flow: 'step_with_missing_navigation_from_operation_result_flow' " + "since for step: 'step1', the result 'FAILURE' of its dependency: 'user.ops.java_op' has no matching navigation");
        compiler.compile(SlangSource.fromFile(resource), path);
    }
    ...
    public void testFlowWithMissingImports() throws Exception {
        ...
        Set<SlangSource> path = new HashSet<>();
        exception.expect(RuntimeException.class);
        exception.expectMessage("Source missing_dependencies_imports_flow has " + "dependencies but no path was given to the compiler");
        compiler.compile(SlangSource.fromFile(resource), path);
    }
    ...
    public void testOperationWithNoActionData() throws Exception {
        ...
        Set<SlangSource> path = new HashSet<>();
        exception.expect(RuntimeException.class);
        exception.expectMessage("Error compiling operation_with_no_action_data. " + "Operation: operation_with_no_action_data has no action data");
        compiler.compile(SlangSource.fromFile(resource), path);
    }
    ...
    public void testDuplicateStepNamesInFlow() throws Exception {
        ...
        Set<SlangSource> path = new HashSet<>();
        exception.expect(RuntimeException.class);
        exception.expectMessage("Step name: 'Step1' appears more than once in the workflow. " + "Each step name in the workflow must be unique");
        compiler.compile(SlangSource.fromFile(resource), path);
    }
