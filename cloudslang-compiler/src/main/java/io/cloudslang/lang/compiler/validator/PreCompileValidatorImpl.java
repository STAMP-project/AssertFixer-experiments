...
public class PreCompileValidatorImpl extends AbstractValidator implements PreCompileValidator {
    ...
    @Override
    public void validateResultsSection(Map<String, Object> executableRawData, String artifact, List<RuntimeException> errors) {
    ...
    private void validateFlow(Flow compiledFlow, ExecutableModellingResult result) {
        if (CollectionUtils.isEmpty(compiledFlow.getWorkflow().getSteps())) {
            ...
            RuntimeException exception = validateNavigation(compiledFlow.getWorkflow().getSteps().getFirst(), compiledFlow.getWorkflow().getSteps(), getStepNames(compiledFlow));
            if (exception != null) {
