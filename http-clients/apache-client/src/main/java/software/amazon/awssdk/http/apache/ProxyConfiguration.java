...
public final class ProxyConfiguration implements ToCopyableBuilder<ProxyConfiguration.Builder, ProxyConfiguration> {
    ...
    private int resolvePort() {
        ...
        if (endpoint != null) {
            ...
            String propertyValue = ProxySystemSetting.PROXY_PORT.getStringValueOrThrow();
            port = Integer.parseInt(propertyValue);
        } else {
            throw new RuntimeException("Proxy port should be provider through endpoint or http.proxyPort system property");
        }
    ...
    public String scheme() {
        return endpoint != null ? endpoint.getScheme()
                                : resolveValue(null, ProxySystemSetting.PROXY_HOST);
    }
