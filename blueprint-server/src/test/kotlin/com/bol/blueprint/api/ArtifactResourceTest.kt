...
class ArtifactResourceTest : AbstractResourceTest() {
    ...
    fun `Can create artifact`() {
        ...
        val result = client.get().uri("$baseUrl/artifact-example.json").exchange().expectStatus().isOk.expectBody(String::class.java).returnResult()
        assertThat(result.responseBody).isEqualTo("""{ "a": "b" }""")
    }
