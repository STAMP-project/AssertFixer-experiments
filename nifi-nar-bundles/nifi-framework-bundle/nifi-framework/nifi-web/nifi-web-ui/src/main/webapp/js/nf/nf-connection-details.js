            ...
            $.when(groupXhr, connectionXhr).done(function (groupResult, connectionResult) {
                ...
                if (connectionResponse.permissions.canRead) {
                    ...
                    $.when(connectionSource, connectionDestination).done(function () {
                        ...
                        nfCommon.populateField('read-only-load-balance-strategy', nfCommon.getComboOptionText(self.loadBalanceStrategyOptions, connection.loadBalanceStrategy));
                        nfCommon.populateField('read-only-load-balance-partition-attribute', connection.loadBalancePartitionAttribute);
                        nfCommon.populateField('read-only-load-balance-compression', nfCommon.getComboOptionText(self.loadBalanceCompressionOptions, connection.loadBalanceCompression));
