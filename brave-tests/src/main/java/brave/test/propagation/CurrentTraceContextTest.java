...
public abstract class CurrentTraceContextTest {
  ...
  protected CurrentTraceContextTest() {
    assertThat(currentSupplier())
        .withFailMessage(currentSupplier() + " should be a static member class")
        .satisfies(c -> {
          assertThat(c.isLocalClass()).isFalse();
          assertThat(Modifier.isPublic(c.getModifiers())).isFalse();
        });

    currentTraceContext = newInstance(currentSupplier());
  }
  ...
  /** Used for classloader leak tests */
  static final String CURRENT_PROPERTY = CurrentTraceContextTest.class.getName() + ".supplier";

  void assertRunIsUnloadableWithCurrentSupplierProperty(Class<? extends Runnable> runnable) {
    System.setProperty(CURRENT_PROPERTY, currentSupplier().getName());
    try {
      assertRunIsUnloadable(getClass().getClassLoader(), runnable);
    } finally {
      System.getProperties().remove(CURRENT_PROPERTY);
    }
  }

  abstract static class CurrentTraceContextRunnable implements Runnable {
    CurrentTraceContext current;

    CurrentTraceContextRunnable() {
      Class<? extends Supplier<CurrentTraceContext>> currentSupplier;
      try {
        currentSupplier = (Class) Class.forName(System.getProperty(CURRENT_PROPERTY));
      } catch (ClassNotFoundException e) {
        throw new AssertionError(e);
      }
      current = newInstance(currentSupplier);
    }
  }

  @Test public void unloadable_unused() {
    assertRunIsUnloadableWithCurrentSupplierProperty(Unused.class);
  }

  static class Unused extends CurrentTraceContextRunnable {
    @Override public void run() {
    }
  }

  @Test public void unloadable_afterScopeClose() {
    assertRunIsUnloadableWithCurrentSupplierProperty(ClosedScope.class);
  }

  static class ClosedScope extends CurrentTraceContextRunnable {
    @Override public void run() {
   ...
   */
  @Test(expected = AssertionError.class) public void notUnloadable_whenScopeLeaked() {
    assertRunIsUnloadableWithCurrentSupplierProperty(LeakedScope.class);
  }

  static class LeakedScope extends CurrentTraceContextRunnable {
    @Override public void run() {
      current.newScope(TraceContext.newBuilder().traceId(1L).spanId(2L).build());
    }
  }

  static <T> T newInstance(Class<? extends Supplier<T>> supplierClass) {
    try {
      Constructor<? extends Supplier<T>> ctor = supplierClass.getDeclaredConstructor();
      ctor.setAccessible(true);

      return ctor.newInstance().get();
