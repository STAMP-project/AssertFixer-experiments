...
public class ActiveMQJMSClient {
   ...
   private static boolean enable1xPrefixes;
   ...
   public static void init() {
      init(ActiveMQJMSClient.class.getName() + ".properties");
   }

   public static void init(String propertiesFile) {
      String value1X = ClassloadingUtil.loadProperty(propertiesFile, "enable1xPrefixes");

      if (value1X == null) {
         value1X = System.getProperty(ActiveMQJMSClient.class.getName() + ".enable1xPrefixes");
      }

      enable1xPrefixes = false;

      if (value1X != null) {
         try {
            enable1xPrefixes = Boolean.parseBoolean(value1X);
         } catch (Throwable e) {
            logger.warn(e);
         }
      }
