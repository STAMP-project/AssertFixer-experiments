...
The servlet filter adapter is packaged as an OSGi bundle, and thus is usable in a generic OSGi environment with HTTP Service and HTTP Whiteboard.
...
First, the adapter needs to be registered as an OSGi component. The easiest way to do that is to extend `org.keycloak.adapters.servlet.KeycloakOIDCFilter` and to decorate it with OSGi annotations:
...
To configure the component, either create a `mypackage.KeycloakFilter.cfg` file under the standard config location for your OSGi runtime:
[source]
