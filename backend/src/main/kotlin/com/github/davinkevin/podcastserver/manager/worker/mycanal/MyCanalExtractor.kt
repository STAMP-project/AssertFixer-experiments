...
class MyCanalExtractor(val htmlService: HtmlService, val jsonService: JsonService) : Extractor {
    ...
    override fun extract(i: Item) =
            htmlService.get(i.url).k()
                    .map { it.body() }
                    ...
                    .map { DownloadingItem(i, listOf(it.hls).toVΛVΓ(), getFileName(i), null) }
                    .getOrElse { throw RuntimeException("Error during extraction of ${i.title} at url ${i.url}") }
