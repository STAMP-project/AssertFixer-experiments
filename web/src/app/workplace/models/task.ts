...
export class Task {
  constructor(public businessProcessId: string,
              ...
              public callbackInfo: Map<string, string>,
...
export function convertToCustomAttributes(): CustomAttribute[] {
  return Object.keys(this.customAttributes).map(k => ({ key: k, value: this.customAttributes[k] }));
}

export function saveCustomAttributes(attributes: CustomAttribute[]): void {
  console.log(attributes);
  this.customAttributes = attributes.filter(att => att.key).reduce((acc, obj) => {
