...
public class ReportAggregator {
    ...
    private void aggregateSet(File currentTestNumDir, Set<MptReportFile> currentReports) throws IOException {
        ...
        BinaryRateWriter binaryRateWriter = null;

        try {
            for (MptReportFile currentReport : currentReports) {
                nodeType = currentReport.getNodeType();

                if (binaryRateWriter == null) {
                    binaryRateWriter = getBinaryRateWriter(nodeType, aggregatedReportRoot);
                }
...
//                }
    ...
    private BinaryRateWriter getBinaryRateWriter(NodeType nodeType, File aggregatedReportRoot) throws IOException {
        BinaryRateWriter binaryRateWriter;
        if (nodeType == NodeType.RECEIVER) {
            binaryRateWriter = new BinaryRateWriter(new File(aggregatedReportRoot, "receiver.dat"),
                    FileHeader.WRITER_DEFAULT_RECEIVER);
        } else {
            binaryRateWriter = new BinaryRateWriter(new File(aggregatedReportRoot, "sender.dat"),
                    FileHeader.WRITER_DEFAULT_SENDER);
        }
        return binaryRateWriter;
    }
