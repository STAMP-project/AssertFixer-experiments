...
public class ActiveMQServerImpl implements ActiveMQServer {
      ...
      for (CoreQueueConfiguration config : queues) {
         try {
            ...
            if (locateQueue(queueName) != null && locateQueue(queueName).getAddress().toString().equals(config.getAddress())) {
               updateQueue(config.getName(), config.getRoutingType(), maxConsumers, config.getPurgeOnNoConsumers(), isExclusive, consumersBeforeDispatch, delayBeforeDispatch, config.getFilterString(), config.getUser());
            } else {
   ...
   public Queue updateQueue(String name,
      ...
      return updateQueue(name, routingType, maxConsumers, purgeOnNoConsumers, exclusive, null, null, null, user);
   }
   ...
   public Queue updateQueue(String name,
                            ...
                            String user) throws Exception {
      final Filter filter = FilterImpl.createFilter(filterString);
      final QueueBinding queueBinding = this.postOffice.updateQueue(new SimpleString(name), routingType, maxConsumers, purgeOnNoConsumers, exclusive, consumersBeforeDispatch, delayBeforeDispatch, filter, SimpleString.toSimpleString(user));
