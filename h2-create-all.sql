-- Generated by ebean unknown at 2018-05-29T15:49:18.059Z
...
create index ix_ec_person_phone_ec_person on ec_person_phone (owner_id);
alter table ec_person_phone add constraint fk_ec_person_phone_ec_person foreign key (owner_id) references ec_person (id) on delete restrict on update restrict;
...
create index ix_ecbl_person_phone_numbers_ecbl_person on ecbl_person_phone_numbers (person_id);
alter table ecbl_person_phone_numbers add constraint fk_ecbl_person_phone_numbers_ecbl_person foreign key (person_id) references ecbl_person (id) on delete restrict on update restrict;
...
create index ix_ecbm_person_phone_numbers_ecbm_person on ecbm_person_phone_numbers (person_id);
alter table ecbm_person_phone_numbers add constraint fk_ecbm_person_phone_numbers_ecbm_person foreign key (person_id) references ecbm_person (id) on delete restrict on update restrict;
...
create index ix_ecm_person_phone_numbers_ecm_person on ecm_person_phone_numbers (ecm_person_id);
alter table ecm_person_phone_numbers add constraint fk_ecm_person_phone_numbers_ecm_person foreign key (ecm_person_id) references ecm_person (id) on delete restrict on update restrict;
...
create index ix_ecs_person_phone_ecs_person on ecs_person_phone (ecs_person_id);
alter table ecs_person_phone add constraint fk_ecs_person_phone_ecs_person foreign key (ecs_person_id) references ecs_person (id) on delete restrict on update restrict;
...
create trigger hx_link_history_upd before update on hx_link for each row begin
    insert into hx_link_history (sys_period_start,sys_period_end,id, name, location, comments, version, when_created, when_modified) values (OLD.sys_period_start, now(6),OLD.id, OLD.name, OLD.location, OLD.comments, OLD.version, OLD.when_created, OLD.when_modified);
