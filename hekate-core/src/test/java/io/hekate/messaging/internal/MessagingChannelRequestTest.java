...
public class MessagingChannelRequestTest extends MessagingServiceTestBase {
    ...
    public void testSendReceive() throws Exception {
        ...
        for (TestChannel from : channels) {
            for (TestChannel to : channels) {
                ...
                assertEquals(msg + "-reply", from.get().forNode(to.nodeId()).request(msg).response());
                assertEquals(msg + "-reply", from.get().forNode(to.nodeId()).request(msg).response(String.class));
    ...
    public void testTopologyChange() throws Throwable {
        ...
        repeat(5, i -> {
            ...
            runAsync(() -> {
                ...
                }).newRequest(joinMsg).execute(joinCallback);
            ...
            runAsync(() -> {
                sender.withLoadBalancer((msg, topology) -> {
                    ...
                    return topology.youngest().id();
                }).newRequest(leaveMsg).execute(leaveCallback);
        ...
        channel.node().cluster().addListener(event -> {
            try {
                ...
                if (event.type() == ClusterEventType.JOIN) {
                    ...
                    send.newRequest("to-self").execute(toSelfCallback);
                } else if (event.type() == ClusterEventType.CHANGE) {
                    get(send.forRemotes().request("to-remote"));

                    send.forRemotes().newRequest("to-remote").execute(toRemoteCallback);
                }
            } catch (Throwable err) {
    ...
    public void testAffinityRouting() throws Throwable {
        ...
        repeat(25, j -> {
            ...
            for (int i = 0; i < 50; i++) {
                get(channel1.get().newRequest("test" + i).withAffinity(j).execute());
            }
