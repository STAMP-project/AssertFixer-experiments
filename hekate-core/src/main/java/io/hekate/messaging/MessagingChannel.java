...
public interface MessagingChannel<T> extends ClusterFilterSupport<MessagingChannel<T>> {
     ...
     * @return New operation.
     *
     * @see Subscribe#execute(ResponseCallback)
     */
     ...
     *
     * <p>
     * If this option is set to {@code true} then receiver of {@link #newSend(Object)} and {@link #broadcast(Object)} operation will send
    ...
    default AggregateFuture<T> aggregate(T message) {
        return newAggregate(message).execute();
    }
    ...
    default BroadcastFuture<T> broadcast(T message) {
        return newBroadcast(message).execute();
    }
     ...
     */
    default SubscribeFuture<T> subscribe(T request, ResponseCallback<T> callback) {
        return newSubscribe(request).execute(callback);
    }
     ...
     */
    default ResponseFuture<T> request(T request) {
    ...
    default SendFuture send(T message) {
        return newSend(message).execute();
    }
}
