 ...
 */
public class FlatCollectByteToObjectIterable<V>
        extends AbstractLazyIterable<V>
{
    private final ByteIterable iterable;
    private final ByteToObjectFunction<? extends Iterable<V>> function;

    public FlatCollectByteToObjectIterable(ByteIterable iterable, ByteToObjectFunction<? extends Iterable<V>> function)
    {
    ...
    {
        this.iterable.forEach((byte each) -> Iterate.forEach(this.function.valueOf(each), procedure));
    }
    ...
    {
        this.iterable.forEach((byte each) -> Iterate.forEachWith(this.function.valueOf(each), procedure, parameter));
    }

    @Override
    public Iterator<V> iterator()
    {
        return new FlatCollectByteIterator<>(this.iterable, this.function);
    }
    ...
    private final class FlatCollectByteIterator<V> implements Iterator<V>
    {
        private final ByteIterator iterator;
        private final ByteToObjectFunction<? extends Iterable<V>> function;
        private Iterator<V> innerIterator = EmptyIterator.getInstance();

        public FlatCollectByteIterator(
                ByteIterable iterable,
                ByteToObjectFunction<? extends Iterable<V>> newFunction)
        {
            this(iterable.byteIterator(), newFunction);
        }

        public FlatCollectByteIterator(
                ByteIterator newIterator,
                ByteToObjectFunction<? extends Iterable<V>> newFunction)
        {
        ...
        public void remove()
        {
            throw new UnsupportedOperationException("Cannot remove from a flatCollectByte iterator");
        }
