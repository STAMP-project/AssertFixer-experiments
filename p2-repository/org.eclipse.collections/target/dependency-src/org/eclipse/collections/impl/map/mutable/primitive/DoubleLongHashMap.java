...
public class DoubleLongHashMap extends AbstractMutableLongValuesMap implements MutableDoubleLongMap, Externalizable, MutableDoubleKeysMap
{
    ...
    {
        if (map instanceof DoubleLongHashMap && ((DoubleLongHashMap) map).occupiedWithSentinels == 0)
        {
            DoubleLongHashMap hashMap = (DoubleLongHashMap) map;
    ...
    private long getForSentinel(double key, long ifAbsent)
    {
        if (isEmptyKey(key))
        {
            if (this.sentinelValues == null || !this.sentinelValues.containsZeroKey)
    ...
    private long slowGetIfAbsent(double key, long ifAbsent)
    {
        int index = this.probe(key);
        if (Double.compare(this.keys[index], key) == 0)
        {
    ...
    private long fastGetIfAbsent(double key, long ifAbsent)
    {
    ...
    private long slowGetIfAbsentTwo(double key, long ifAbsent)
    {
    ...
    private class InternalLongIterator implements MutableLongIterator
    {
        ...
        public void remove()
        {
            ...
            DoubleLongHashMap.this.removeKey(this.lastKey);
            this.count--;
    ...
    private class KeySet extends AbstractMutableDoubleKeySet
    {
        ...
        {
            return DoubleLongHashMap.this;
        }

        @Override
        protected SentinelValues getSentinelValues()
        {
            return DoubleLongHashMap.this.sentinelValues;
        }

        @Override
        protected double getKeyAtIndex(int index)
        {
            return DoubleLongHashMap.this.keys[index];
        }

        @Override
        protected int getTableSize()
        {
            return DoubleLongHashMap.this.keys.length;
        }
        ...
        public boolean retainAll(DoubleIterable source)
        {
            ...
            DoubleLongHashMap retained = DoubleLongHashMap.this.select((double key, long value) -> sourceSet.contains(key));
        ...
        {
            DoubleLongHashMap.this.copyKeysOnWrite = true;
            boolean containsZeroKey = false;
            boolean containsOneKey = false;
            if (DoubleLongHashMap.this.sentinelValues != null)
            {
                containsZeroKey = DoubleLongHashMap.this.sentinelValues.containsZeroKey;
                containsOneKey = DoubleLongHashMap.this.sentinelValues.containsOneKey;
            }
            return new ImmutableDoubleMapKeySet(DoubleLongHashMap.this.keys, DoubleLongHashMap.this.occupiedWithData, containsZeroKey, containsOneKey);
        }
    ...
    private class ValuesCollection extends AbstractLongValuesCollection
    {
        ...
        public boolean retainAll(LongIterable source)
        {
            ...
            DoubleLongHashMap retained = DoubleLongHashMap.this.select((double key, long value) -> sourceSet.contains(value));
            if (retained.size() != oldSize)
            {
                DoubleLongHashMap.this.keys = retained.keys;
                DoubleLongHashMap.this.values = retained.values;
    ...
    private class KeyValuesView extends AbstractLazyIterable<DoubleLongPair>
    {
        @Override
        public void each(Procedure<? super DoubleLongPair> procedure)
        {
            if (DoubleLongHashMap.this.sentinelValues != null)
            {
        ...
        @Override
        public <P> void forEachWith(Procedure2<? super DoubleLongPair, ? super P> procedure, P parameter)
        {
            if (DoubleLongHashMap.this.sentinelValues != null)
            {
                if (DoubleLongHashMap.this.sentinelValues.containsZeroKey)
        ...
        @Override
        public Iterator<DoubleLongPair> iterator()
        {
            return new InternalKeyValuesIterator();
        }

        public class InternalKeyValuesIterator implements Iterator<DoubleLongPair>
        {
            private int count;
            private int position;
            private boolean handledZero;
