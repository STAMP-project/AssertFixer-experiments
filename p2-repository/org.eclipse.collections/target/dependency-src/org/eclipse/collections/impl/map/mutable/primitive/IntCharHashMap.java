...
public class IntCharHashMap extends AbstractMutableCharValuesMap implements MutableIntCharMap, Externalizable, MutableIntKeysMap
{
    ...
    private class InternalCharIterator implements MutableCharIterator
    {
        ...
        public char next()
        {
            ...
            if (!this.handledZero)
            {
                ...
                if (IntCharHashMap.this.containsKey(EMPTY_KEY))
                {
                    this.lastKey = EMPTY_KEY;
                    return IntCharHashMap.this.get(EMPTY_KEY);
                }
            }
            if (!this.handledOne)
            {
                this.handledOne = true;
                if (IntCharHashMap.this.containsKey(REMOVED_KEY))
                {
                    this.lastKey = REMOVED_KEY;
                    return IntCharHashMap.this.get(REMOVED_KEY);
                }
            }
    ...
    private class KeySet extends AbstractMutableIntKeySet
    {
        ...
        public boolean retainAll(IntIterable source)
        {
            ...
            IntCharHashMap retained = IntCharHashMap.this.select((int key, char value) -> sourceSet.contains(key));
            if (retained.size() != oldSize)
            {
                IntCharHashMap.this.keys = retained.keys;
                IntCharHashMap.this.values = retained.values;
        ...
        public IntSet freeze()
        {
            ...
            {
                containsZeroKey = IntCharHashMap.this.sentinelValues.containsZeroKey;
                containsOneKey = IntCharHashMap.this.sentinelValues.containsOneKey;
            }
    ...
    private class ValuesCollection extends AbstractCharValuesCollection
    {
        ...
        public boolean remove(char item)
        {
            ...
            if (IntCharHashMap.this.sentinelValues != null)
            {
                if (IntCharHashMap.this.sentinelValues.containsZeroKey && item == IntCharHashMap.this.sentinelValues.zeroValue)
                {
                    IntCharHashMap.this.removeKey(EMPTY_KEY);
                }
                if (IntCharHashMap.this.sentinelValues.containsOneKey && item == IntCharHashMap.this.sentinelValues.oneValue)
                {
                    IntCharHashMap.this.removeKey(REMOVED_KEY);
                }
            }
            for (int i = 0; i < IntCharHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(IntCharHashMap.this.keys[i]) && item == IntCharHashMap.this.values[i])
                {
        ...
        public boolean retainAll(CharIterable source)
        {
            ...
            IntCharHashMap retained = IntCharHashMap.this.select((int key, char value) -> sourceSet.contains(value));
            if (retained.size() != oldSize)
            {
                IntCharHashMap.this.keys = retained.keys;
                IntCharHashMap.this.values = retained.values;
    ...
    private class KeyValuesView extends AbstractLazyIterable<IntCharPair>
    {
        @Override
        public void each(Procedure<? super IntCharPair> procedure)
        {
            if (IntCharHashMap.this.sentinelValues != null)
            {
                ...
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, IntCharHashMap.this.sentinelValues.zeroValue));
                }
                if (IntCharHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, IntCharHashMap.this.sentinelValues.oneValue));
                }
            }
            for (int i = 0; i < IntCharHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(IntCharHashMap.this.keys[i]))
                {
        ...
        public void forEachWithIndex(ObjectIntProcedure<? super IntCharPair> objectIntProcedure)
        {
            ...
            if (IntCharHashMap.this.sentinelValues != null)
            {
                if (IntCharHashMap.this.sentinelValues.containsZeroKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(EMPTY_KEY, IntCharHashMap.this.sentinelValues.zeroValue), index);
                    index++;
                }
                if (IntCharHashMap.this.sentinelValues.containsOneKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(REMOVED_KEY, IntCharHashMap.this.sentinelValues.oneValue), index);
                    index++;
                }
            }
            for (int i = 0; i < IntCharHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(IntCharHashMap.this.keys[i]))
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(IntCharHashMap.this.keys[i], IntCharHashMap.this.values[i]), index);
                    index++;
                }
            }
        }
        ...
        public <P> void forEachWith(Procedure2<? super IntCharPair, ? super P> procedure, P parameter)
        {
            if (IntCharHashMap.this.sentinelValues != null)
            {
                if (IntCharHashMap.this.sentinelValues.containsZeroKey)
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, IntCharHashMap.this.sentinelValues.zeroValue), parameter);
                }
                if (IntCharHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, IntCharHashMap.this.sentinelValues.oneValue), parameter);
                }
            }
            for (int i = 0; i < IntCharHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(IntCharHashMap.this.keys[i]))
                {
                    procedure.value(PrimitiveTuples.pair(IntCharHashMap.this.keys[i], IntCharHashMap.this.values[i]), parameter);
                }
            }
        }
        ...
        public class InternalKeyValuesIterator implements Iterator<IntCharPair>
        {
            ...
            public IntCharPair next()
            {
                ...
                if (!this.handledZero)
                {
                    ...
                    if (IntCharHashMap.this.containsKey(EMPTY_KEY))
                    {
                        return PrimitiveTuples.pair(EMPTY_KEY, IntCharHashMap.this.sentinelValues.zeroValue);
                    }
                }
                if (!this.handledOne)
                {
                    ...
                    if (IntCharHashMap.this.containsKey(REMOVED_KEY))
                    {
                        return PrimitiveTuples.pair(REMOVED_KEY, IntCharHashMap.this.sentinelValues.oneValue);
                    }
                }
