...
public class IntLongHashMap extends AbstractMutableLongValuesMap implements MutableIntLongMap, Externalizable, MutableIntKeysMap
{
    ...
    {
        if (map instanceof IntLongHashMap && ((IntLongHashMap) map).occupiedWithSentinels == 0)
        {
            IntLongHashMap hashMap = (IntLongHashMap) map;
    ...
    private void addKeyValueAtIndex(int key, long value, int index)
    {
    ...
    private long getForSentinel(int key, long ifAbsent)
    {
        if (isEmptyKey(key))
        {
            if (this.sentinelValues == null || !this.sentinelValues.containsZeroKey)
    ...
    private long slowGetIfAbsent(int key, long ifAbsent)
    {
        int index = this.probe(key);
        if (this.keys[index] == key)
        {
    ...
    private long fastGetIfAbsent(int key, long ifAbsent)
    {
    ...
    private long slowGetIfAbsentTwo(int key, long ifAbsent)
    {
    ...
    {
        IntLongHashMap result = new IntLongHashMap();
    ...
    private class InternalLongIterator implements MutableLongIterator
    {
        ...
        public long next()
        {
            ...
            if (!this.handledZero)
            {
                ...
                if (IntLongHashMap.this.containsKey(EMPTY_KEY))
                {
                    this.lastKey = EMPTY_KEY;
                    return IntLongHashMap.this.get(EMPTY_KEY);
                }
            }
            if (!this.handledOne)
            {
                this.handledOne = true;
                if (IntLongHashMap.this.containsKey(REMOVED_KEY))
                {
                    this.lastKey = REMOVED_KEY;
                    return IntLongHashMap.this.get(REMOVED_KEY);
                }
            }
            int[] keys = IntLongHashMap.this.keys;
            while (!isNonSentinel(keys[this.position]))
            {
                this.position++;
            }
            this.lastKey = keys[this.position];
            long result = IntLongHashMap.this.values[this.position];
            this.position++;
    ...
    private class KeysView extends AbstractLazyIntIterable
    {
        ...
        {
            IntLongHashMap.this.forEachKey(procedure);
        }
    }
    ...
    private class KeySet extends AbstractMutableIntKeySet
    {
        ...
        {
            return IntLongHashMap.this.keys[index];
        }
        ...
        public boolean retainAll(IntIterable source)
        {
            ...
            IntLongHashMap retained = IntLongHashMap.this.select((int key, long value) -> sourceSet.contains(key));
            if (retained.size() != oldSize)
            {
                IntLongHashMap.this.keys = retained.keys;
                IntLongHashMap.this.values = retained.values;
                IntLongHashMap.this.sentinelValues = retained.sentinelValues;
                IntLongHashMap.this.occupiedWithData = retained.occupiedWithData;
                IntLongHashMap.this.occupiedWithSentinels = retained.occupiedWithSentinels;
                return true;
            }
            return false;
        }
        ...
        public IntSet freeze()
        {
            ...
            {
                containsZeroKey = IntLongHashMap.this.sentinelValues.containsZeroKey;
                containsOneKey = IntLongHashMap.this.sentinelValues.containsOneKey;
            }
            return new ImmutableIntMapKeySet(IntLongHashMap.this.keys, IntLongHashMap.this.occupiedWithData, containsZeroKey, containsOneKey);
        }
    ...
    private class ValuesCollection extends AbstractLongValuesCollection
    {
        ...
        public boolean remove(long item)
        {
            ...
            if (IntLongHashMap.this.sentinelValues != null)
            {
                if (IntLongHashMap.this.sentinelValues.containsZeroKey && item == IntLongHashMap.this.sentinelValues.zeroValue)
                {
                    IntLongHashMap.this.removeKey(EMPTY_KEY);
                }
                if (IntLongHashMap.this.sentinelValues.containsOneKey && item == IntLongHashMap.this.sentinelValues.oneValue)
                {
                    IntLongHashMap.this.removeKey(REMOVED_KEY);
                }
            }
            for (int i = 0; i < IntLongHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(IntLongHashMap.this.keys[i]) && item == IntLongHashMap.this.values[i])
                {
        ...
        public boolean retainAll(LongIterable source)
        {
            ...
            IntLongHashMap retained = IntLongHashMap.this.select((int key, long value) -> sourceSet.contains(value));
            if (retained.size() != oldSize)
            {
                IntLongHashMap.this.keys = retained.keys;
                IntLongHashMap.this.values = retained.values;
    ...
    private class KeyValuesView extends AbstractLazyIterable<IntLongPair>
    {
        @Override
        public void each(Procedure<? super IntLongPair> procedure)
        {
            if (IntLongHashMap.this.sentinelValues != null)
            {
                ...
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, IntLongHashMap.this.sentinelValues.zeroValue));
                }
                if (IntLongHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, IntLongHashMap.this.sentinelValues.oneValue));
                }
            }
            for (int i = 0; i < IntLongHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(IntLongHashMap.this.keys[i]))
                {
        ...
        public void forEachWithIndex(ObjectIntProcedure<? super IntLongPair> objectIntProcedure)
        {
            ...
            if (IntLongHashMap.this.sentinelValues != null)
            {
                if (IntLongHashMap.this.sentinelValues.containsZeroKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(EMPTY_KEY, IntLongHashMap.this.sentinelValues.zeroValue), index);
                    index++;
                }
                if (IntLongHashMap.this.sentinelValues.containsOneKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(REMOVED_KEY, IntLongHashMap.this.sentinelValues.oneValue), index);
                    index++;
                }
            }
            for (int i = 0; i < IntLongHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(IntLongHashMap.this.keys[i]))
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(IntLongHashMap.this.keys[i], IntLongHashMap.this.values[i]), index);
                    index++;
                }
            }
        }
        ...
        @Override
        public <P> void forEachWith(Procedure2<? super IntLongPair, ? super P> procedure, P parameter)
        {
            if (IntLongHashMap.this.sentinelValues != null)
            {
                if (IntLongHashMap.this.sentinelValues.containsZeroKey)
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, IntLongHashMap.this.sentinelValues.zeroValue), parameter);
                }
                if (IntLongHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, IntLongHashMap.this.sentinelValues.oneValue), parameter);
                }
            }
            for (int i = 0; i < IntLongHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(IntLongHashMap.this.keys[i]))
                {
                    procedure.value(PrimitiveTuples.pair(IntLongHashMap.this.keys[i], IntLongHashMap.this.values[i]), parameter);
                }
            }
        }
        ...
        public class InternalKeyValuesIterator implements Iterator<IntLongPair>
        {
            ...
            public IntLongPair next()
            {
                ...
                if (!this.handledZero)
                {
                    ...
                    if (IntLongHashMap.this.containsKey(EMPTY_KEY))
                    {
                        return PrimitiveTuples.pair(EMPTY_KEY, IntLongHashMap.this.sentinelValues.zeroValue);
                    }
                }
                if (!this.handledOne)
                {
                    ...
                    if (IntLongHashMap.this.containsKey(REMOVED_KEY))
                    {
                        return PrimitiveTuples.pair(REMOVED_KEY, IntLongHashMap.this.sentinelValues.oneValue);
                    }
                }
                ...
                IntLongPair result = PrimitiveTuples.pair(keys[this.position], IntLongHashMap.this.values[this.position]);
                this.position++;
                return result;
            }
