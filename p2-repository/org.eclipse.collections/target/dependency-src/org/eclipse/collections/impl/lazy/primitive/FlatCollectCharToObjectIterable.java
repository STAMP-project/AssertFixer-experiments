        ...
        extends AbstractLazyIterable<V>
{
    ...
    {
        this.iterable.forEach((char each) -> Iterate.forEach(this.function.valueOf(each), procedure));
    }
    ...
    {
        this.iterable.forEach((char each) -> Iterate.forEachWith(this.function.valueOf(each), procedure, parameter));
    }
    ...
    private final class FlatCollectCharIterator<V> implements Iterator<V>
    {
        private final CharIterator iterator;
        private final CharToObjectFunction<? extends Iterable<V>> function;
        private Iterator<V> innerIterator = EmptyIterator.getInstance();

        public FlatCollectCharIterator(
                CharIterable iterable,
                CharToObjectFunction<? extends Iterable<V>> newFunction)
        {
            this(iterable.charIterator(), newFunction);
        }

        public FlatCollectCharIterator(
                CharIterator newIterator,
                CharToObjectFunction<? extends Iterable<V>> newFunction)
        {
