...
public class CharDoubleHashMap extends AbstractMutableDoubleValuesMap implements MutableCharDoubleMap, Externalizable, MutableCharKeysMap
{
    ...
    {
        if (map instanceof CharDoubleHashMap && ((CharDoubleHashMap) map).occupiedWithSentinels == 0)
        {
            CharDoubleHashMap hashMap = (CharDoubleHashMap) map;
    ...
    private class InternalDoubleIterator implements MutableDoubleIterator
    {
        ...
        public double next()
        {
            ...
            if (!this.handledZero)
            {
                ...
                if (CharDoubleHashMap.this.containsKey(EMPTY_KEY))
                {
                    this.lastKey = EMPTY_KEY;
                    return CharDoubleHashMap.this.get(EMPTY_KEY);
                }
            }
            if (!this.handledOne)
            {
                this.handledOne = true;
                if (CharDoubleHashMap.this.containsKey(REMOVED_KEY))
                {
                    this.lastKey = REMOVED_KEY;
                    return CharDoubleHashMap.this.get(REMOVED_KEY);
                }
            }
    ...
    private class KeySet extends AbstractMutableCharKeySet
    {
        ...
        public boolean retainAll(CharIterable source)
        {
            ...
            CharDoubleHashMap retained = CharDoubleHashMap.this.select((char key, double value) -> sourceSet.contains(key));
            if (retained.size() != oldSize)
            {
                CharDoubleHashMap.this.keys = retained.keys;
                CharDoubleHashMap.this.values = retained.values;
        ...
        public CharSet freeze()
        {
            ...
            {
                containsZeroKey = CharDoubleHashMap.this.sentinelValues.containsZeroKey;
                containsOneKey = CharDoubleHashMap.this.sentinelValues.containsOneKey;
            }
    ...
    private class ValuesCollection extends AbstractDoubleValuesCollection
    {
        ...
        public boolean remove(double item)
        {
            ...
            if (CharDoubleHashMap.this.sentinelValues != null)
            {
                ...
                {
                    CharDoubleHashMap.this.removeKey(EMPTY_KEY);
                }
                ...
                {
                    CharDoubleHashMap.this.removeKey(REMOVED_KEY);
                }
            }
            for (int i = 0; i < CharDoubleHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(CharDoubleHashMap.this.keys[i]) && Double.compare(item, CharDoubleHashMap.this.values[i]) == 0)
                {
        ...
        public boolean retainAll(DoubleIterable source)
        {
            ...
            CharDoubleHashMap retained = CharDoubleHashMap.this.select((char key, double value) -> sourceSet.contains(value));
            if (retained.size() != oldSize)
            {
                CharDoubleHashMap.this.keys = retained.keys;
                CharDoubleHashMap.this.values = retained.values;
    ...
    private class KeyValuesView extends AbstractLazyIterable<CharDoublePair>
    {
        @Override
        public void each(Procedure<? super CharDoublePair> procedure)
        {
            if (CharDoubleHashMap.this.sentinelValues != null)
            {
                ...
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, CharDoubleHashMap.this.sentinelValues.zeroValue));
                }
                if (CharDoubleHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, CharDoubleHashMap.this.sentinelValues.oneValue));
                }
            }
            for (int i = 0; i < CharDoubleHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(CharDoubleHashMap.this.keys[i]))
                {
        ...
        public void forEachWithIndex(ObjectIntProcedure<? super CharDoublePair> objectIntProcedure)
        {
            ...
            if (CharDoubleHashMap.this.sentinelValues != null)
            {
                if (CharDoubleHashMap.this.sentinelValues.containsZeroKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(EMPTY_KEY, CharDoubleHashMap.this.sentinelValues.zeroValue), index);
                    index++;
                }
                if (CharDoubleHashMap.this.sentinelValues.containsOneKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(REMOVED_KEY, CharDoubleHashMap.this.sentinelValues.oneValue), index);
                    index++;
                }
            }
            for (int i = 0; i < CharDoubleHashMap.this.keys.length; i++)
        ...
        public <P> void forEachWith(Procedure2<? super CharDoublePair, ? super P> procedure, P parameter)
        {
            if (CharDoubleHashMap.this.sentinelValues != null)
            {
                if (CharDoubleHashMap.this.sentinelValues.containsZeroKey)
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, CharDoubleHashMap.this.sentinelValues.zeroValue), parameter);
                }
                if (CharDoubleHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, CharDoubleHashMap.this.sentinelValues.oneValue), parameter);
                }
            }
            for (int i = 0; i < CharDoubleHashMap.this.keys.length; i++)
            {
        ...
        public class InternalKeyValuesIterator implements Iterator<CharDoublePair>
        {
            ...
            public CharDoublePair next()
            {
                ...
                if (!this.handledZero)
                {
                    ...
                    if (CharDoubleHashMap.this.containsKey(EMPTY_KEY))
                    {
                        return PrimitiveTuples.pair(EMPTY_KEY, CharDoubleHashMap.this.sentinelValues.zeroValue);
                    }
                }
                if (!this.handledOne)
                {
                    ...
                    if (CharDoubleHashMap.this.containsKey(REMOVED_KEY))
                    {
                        return PrimitiveTuples.pair(REMOVED_KEY, CharDoubleHashMap.this.sentinelValues.oneValue);
                    }
                }
