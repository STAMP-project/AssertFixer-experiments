 ...
 * @since 4.0.
 */
final class ImmutableShortObjectSingletonMap<V> extends AbstractImmutableShortObjectMap<V> implements Serializable
{
    ...
    @Override
    public ImmutableShortObjectMap<V> select(ShortObjectPredicate<? super V> predicate)
    {
        return predicate.accept(this.key1, this.value1) ? ShortObjectHashMap.newWithKeysValues(this.key1, this.value1).toImmutable()
                : (ImmutableShortObjectMap<V>) ImmutableShortObjectEmptyMap.INSTANCE;
    }

    @Override
    public ImmutableShortObjectMap<V> reject(ShortObjectPredicate<? super V> predicate)
    {
        return predicate.accept(this.key1, this.value1) ? (ImmutableShortObjectMap<V>) ImmutableShortObjectEmptyMap.INSTANCE
    ...
    {
        return ObjectShortMaps.immutable.with(this.value1, this.key1);
    }
    ...
    public boolean equals(Object obj)
    {
        ...
        ShortObjectMap<V> map = (ShortObjectMap<V>) obj;
        if (map.size() != 1)
        {
            return false;
        }
    ...
    @Override
    public ImmutableShortObjectMap<V> newWithKeyValue(short key, V value)
    {
        return ShortObjectHashMap.newWithKeysValues(this.key1, this.value1, key, value).toImmutable();
    }

    @Override
    public ImmutableShortObjectMap<V> newWithoutKey(short key)
    {
        return this.key1 == key ? (ImmutableShortObjectMap<V>) ImmutableShortObjectEmptyMap.INSTANCE : this;
    }

    @Override
    public ImmutableShortObjectMap<V> newWithoutAllKeys(ShortIterable keys)
    {
        return keys.contains(this.key1) ? (ImmutableShortObjectMap<V>) ImmutableShortObjectEmptyMap.INSTANCE : this;
    }
    ...
    {
        return new UnmodifiableIteratorAdapter<>(ShortObjectHashMap.newWithKeysValues(this.key1, this.value1).iterator());
    }

    private Object writeReplace()
    {
        return new ImmutableShortObjectMapSerializationProxy<V>(this);
    }
}
