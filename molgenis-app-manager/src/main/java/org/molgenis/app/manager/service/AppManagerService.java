...
public interface AppManagerService
{
	 ...
	 * @throws IOException if creating the temporary directory or extracting the file fail
	 */
	Path uploadApp(InputStream zipData, String zipFileName, String formFieldName) throws IOException;

	/**
	 * Check the app-configuration and obtain the {@link AppConfig}
	 * if the configuration is valid.
	 * Deletes the tempDir if a problem occurs.
	 *
	 * @param tempDir temporary directory with uploaded app-content
	 * @return appConfig
	 * @throws IOException if the temp dir cannot be deleted
	 */
	AppConfig checkAndObtainConfig(Path tempDir, String configContent) throws IOException;
	 ...
	 *
	 * @param filePath file path relative to the file store root
	 * @return UTF-8 file-content
	 */
	String extractFileContent(Path filePath) throws IOException;
}
