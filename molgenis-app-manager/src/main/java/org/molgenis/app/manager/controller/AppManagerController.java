...
public class AppManagerController extends PluginController
{
	...
	public void uploadApp(@RequestParam("file") MultipartFile multipartFile)
	{
		...
		{
			Path tempDir = appManagerService.uploadApp(fileInputStream, filename, formFieldName);
			String configContent = appManagerService.extractFileContent(tempDir.resolve(ZIP_CONFIG_FILE));
			AppConfig appConfig = appManagerService.checkAndObtainConfig(tempDir, configContent);
			String htmlTemplate = appManagerService.extractFileContent(
					Paths.get(APPS_DIR, appConfig.getUri(), ZIP_INDEX_FILE));
			appManagerService.configureApp(appConfig, htmlTemplate);
		}
		catch (IOException err)
		{
			throw new CouldNotUploadAppException(filename, err);
		}
	}
}
