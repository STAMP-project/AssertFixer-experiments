...
public final class Accessors {
    ...
    public static AttributeAccessor forEntityMapping(EntityViewManagerImpl evm, MethodAttribute<?, ?> attribute) {
        if (attribute instanceof MappingAttribute<?, ?>) {
            return forEntityMapping(evm, evm.getCriteriaBuilderFactory().getService(EntityMetamodel.class), attribute.getDeclaringType().getEntityClass(), ((MappingAttribute<?, ?>) attribute).getMapping());
        } else {
            return null;
        }
    }

    public static AttributeAccessor forEntityMapping(EntityViewManagerImpl evm, EntityMetamodel metamodel, Class<?> entityClass, String mapping) {
        AttributePath path = evm.getJpaProvider().getJpaMetamodelAccessor().getBasicAttributePath(metamodel, metamodel.managedType(entityClass), mapping);
