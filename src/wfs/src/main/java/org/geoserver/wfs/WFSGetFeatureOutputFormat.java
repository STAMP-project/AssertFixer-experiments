...
public abstract class WFSGetFeatureOutputFormat extends WFSResponse {
    ...
    protected int[] getNumDecimalsEncodeMeasures(
            List featureCollections, GeoServer geoServer, Catalog catalog) {
        int numDecimals = -1;
        int encodeMeasures = 1;
        for (int i = 0; i < featureCollections.size(); i++) {
            ...
            if (meta instanceof FeatureTypeInfo) {
                ...
                int ftiDecimals = fti.getNumDecimals();
                if (ftiDecimals > 0) {
                    numDecimals =
                            numDecimals == -1 ? ftiDecimals : Math.max(numDecimals, ftiDecimals);
                }
                if (!fti.getEncodeMeasures()) {
                    // measures will be encoded
                    encodeMeasures = 0;
                }
            }
        }
        ...
        return new int[] {numDecimals, encodeMeasures};
    }
