...
public class DockerService {
  ...
  private ContainerParameters params;
  ...
  DockerService(ContainerParameters params) {
    ...
    ContainerConfig containerConfig =
      createContainerConfig(
        params.imageName, params.ports, params.cmd, params.envs, params.containerToLink,
        params.pathToConfig, params.pathToResource, params.resourceName);
    ...
    try {
      ...
      val test = dockerClient.inspectContainer(container.id());
  ...
  protected ContainerConfig createContainerConfig(
    String imageName, Map<Integer, Integer> ports, String cmd, String[] envs,
    String containerToLink, String pathToConfig, String pathToResource, String resourceName) {
    Map<String, List<PortBinding>> portBindings = new HashMap<>();

    Set<String> exposedPorts = ports.values().stream().map(Object::toString)
      .collect(Collectors.toSet());

    for (Map.Entry<Integer, Integer> port : ports.entrySet()) {
      List<PortBinding> hostPorts = Collections
        .singletonList(PortBinding.of("0.0.0.0", port.getKey().toString()));
    ...
    if (containerToLink != null && pathToConfig != null && resourceName != null) {
      hostConfig = HostConfig.builder().portBindings(portBindings).links(containerToLink)
        .appendBinds(
          Bind.from(pathToConfig)
            .to("/config").readOnly(true).build())
        .appendBinds(
          Bind.from(pathToResource).to(resourceName)
            .build()).build();
    } else if (containerToLink != null && pathToConfig != null) {
      hostConfig = HostConfig.builder().portBindings(portBindings).links(containerToLink)
        .appendBinds(
          Bind.from(pathToConfig)
            .to("/config").readOnly(true).build()).build();
    } else {
