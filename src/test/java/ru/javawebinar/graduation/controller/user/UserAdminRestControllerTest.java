package ru.javawebinar.graduation.controller.user;
...
import static ru.javawebinar.graduation.controller.ExceptionInfoHandler.EXCEPTION_DUPLICATE_EMAIL;
...
class UserAdminRestControllerTest extends AbstractControllerTest {
    ...
    void testUpdateHtmlUnsafe() throws Exception {
        User invalid = new User(USER_ID, "Updated", "<script>alert(123)</script>", "newPass", Role.ROLE_USER, Role.ROLE_ADMIN);
        mockMvc.perform(put(REST_URL + USER_ID)
                .contentType(MediaType.APPLICATION_JSON)
