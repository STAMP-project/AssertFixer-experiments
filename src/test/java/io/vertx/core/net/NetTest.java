...
public class NetTest extends VertxTestBase {
  ...
  @Test
  public void testWriteHandlerQueueFull() {
    final AtomicInteger writeHandlerCounter = new AtomicInteger(0);
    Handler<AsyncResult<Void>> writeHandler =  (v) -> {
      if(v.succeeded()) {
        writeHandlerCounter.incrementAndGet();
      } else {
        fail(v.cause());
      }
      testComplete();
    ...
    Buffer sent = TestUtils.randomBuffer(100);
    testEcho(sock -> {
        sock.setWriteQueueMaxSize(100);
        assertFalse(sock.writeQueueFull());
        sock.write(sent, writeHandler);
        assertTrue(sock.writeQueueFull());
        sock.write(sent, writeHandler);
      },
      buff -> assertEquals(sent, buff), sent.length());
    assertEquals(1, writeHandlerCounter.get());
  }

  @Test
  public void testWriteHandlerPause() {
    final AtomicInteger writeHandlerCounter = new AtomicInteger(0);
    Handler<AsyncResult<Void>> writeHandler =  (v) -> {
      if(v.succeeded()) {
        writeHandlerCounter.incrementAndGet();
