...
public class CleanupTest extends KafkaClusterTestBase {
  ...
  private Vertx vertx;
  private int numNetworkProducerThread;
  private int numNetworkConsumerThread;
  ...
  public void beforeTest() {
    ...
    numNetworkProducerThread = countThreads("kafka-producer-network-thread");
    numNetworkConsumerThread = countThreads("kafka-consumer-network-thread");
  }
  ...
  public void testSharedProducer(TestContext ctx) {
    ...
    waitUntil(() -> countThreads("kafka-producer-network-thread") == numNetworkProducerThread);
  }

  private void close(TestContext ctx, LinkedList<KafkaProducer<String, String>> producers, Runnable doneHandler) {
    close(numNetworkProducerThread, ctx, producers, doneHandler);
  }
  ...
  public void testCleanupInConsumer(TestContext ctx) {
    ...
    vertx.deployVerticle(new AbstractVerticle() {
      ...
      public void start(Future<Void> fut) {
        ...
        consumer.handler(record -> {
          ...
          vertx.undeploy(context.deploymentID(), ctx.asyncAssertSuccess(v -> async.countDown()));
        });
    ...
    async.awaitSuccess(10000);
    waitUntil(() -> countThreads("vert.x-kafka-consumer-thread") == numNetworkConsumerThread);
  }
  ...
  public void testUndeployUnassignedConsumer(TestContext ctx) {
    ...
    vertx.deployVerticle(new AbstractVerticle() {
      @Override
      public void start() throws Exception {
        // We don't assign it, check close hook will catch it
        KafkaConsumer<String, String> consumer = KafkaConsumer.create(vertx, config);
        vertx.undeploy(context.deploymentID(), ctx.asyncAssertSuccess(v -> async.complete()));
      }
