...
public class AsynchronousProcessingTest {
    ...
    public void completionCallbacksCanBeRegistered() throws Exception {
        ...
        class Sample {
            @GET
            public void go(@Suspended AsyncResponse ar, @QueryParam("retryDate") Long retryDate, @QueryParam("retrySeconds") Integer retrySeconds) throws InterruptedException {
        ...
        this.server = httpServer().addHandler(restHandler(new Sample())).start();
        OkHttpClient impatientClient = ClientUtils.client.newBuilder().readTimeout(500, TimeUnit.MILLISECONDS).build();
