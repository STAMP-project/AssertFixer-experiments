...
public class AggregateChange<T> {
	...
	static void setId(RelationalMappingContext context, RelationalConverter converter,
		...
		if (leafProperty.isQualified()) {
			...
			// we have to explicitely convert the id, because the ConvertingPropertyAccessor currently loses its conversion
			// capabilities for non direct property access.
			Object convertedId = converter.readValue(generatedId, pathToId.getRequiredLeafProperty().getTypeInformation());
			propertyAccessor.setProperty(pathToId,
					convertedId);
		}
	}
