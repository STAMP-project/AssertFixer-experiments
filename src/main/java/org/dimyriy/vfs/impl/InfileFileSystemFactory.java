 ...
 * This class is a singleton since it's never a good idea to have more than one instance of this class in an application.
...
public class InfileFileSystemFactory implements StorageFileSystemFactory {
  ...
  public InfileFileSystem loadFileSystemFromCache(@Nonnull final String absolutePathToParentDirectory, final String filename) {
    return guardedCreator.inCreateContext(() -> {
  ...
  @Override
  public InfileFileSystem createNewFileSystemOfSizeInBytes(@Nonnull final String absolutePathToParentDirectory,
                                                           @Nonnull final String filename,
                                                           final long fsSizeInBytes,
                                                           final int clusterSize) {
    if (fsSizeInBytes % StorageConstants.getDefaultClusterSizeInBytes() != 0) {
      throw new InitializationException("fsSizeInBytes should be a multiplier of cluster size [" + StorageConstants.getDefaultClusterSizeInBytes() + "]");
    }
    if (fsSizeInBytes / clusterSize > Integer.MAX_VALUE) {
      throw new InitializationException("fsSizeInBytes should not be larger than Integer.MAX_VALUE * clusterSize");
    }
    return createNewFileSystem(absolutePathToParentDirectory, filename, (int) fsSizeInBytes / clusterSize, clusterSize);
  }
