...
public class LruPool<T extends Closeable> implements Closeable {
    ...
    try {
      processor.inWriteContext(() -> {
        ...
        if (thrown.getValue() != null) {
          ...
          throw new StorageIOException(thrown.getValue());
        }
        LOGGER.trace("Closing LruPool finished");
      });
    } catch (final StorageIOException e) {
      throw (IOException) e.getCause();
    }
  }

  public T borrowObject() {
    return processor.inCreateContext(() -> {
  ...
  public void putObject(@Nonnull final T object) {
    processor.inWriteContext(() -> lruSet.add(object));
  }
}
