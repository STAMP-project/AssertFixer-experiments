...
public interface GuarderProcessor {
  ...
  <T> T inReadContext(@Nonnull Supplier<T> read);

  void inWriteContext(NoArgVoid write);

  <T> T inCreateContext(@Nonnull Supplier<T> create);

  class Blocking implements GuarderProcessor {
    private final Lock guard = new ReentrantLock();

    @Override
    public <T> T inReadContext(@Nonnull final Supplier<T> read) {
      guard.lock();
      try {
        return read.get();
      } finally {
    ...
    @Override
    public void inWriteContext(final NoArgVoid write) {
      guard.lock();
      try {
        write.apply();
      } finally {
    ...
    @Override
    public <T> T inCreateContext(@Nonnull final Supplier<T> create) {
      guard.lock();
      try {
        return create.get();
      } finally {
