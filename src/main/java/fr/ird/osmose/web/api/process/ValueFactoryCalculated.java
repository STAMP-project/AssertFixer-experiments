        ...
        this.factoryMap = new HashMap<String, ValueFactory>() {
            ...
            private ValueFactory calculateCriticalPredationEfficiency() {
                return (name, group) -> {
                    String value = null;
                    try {
                        String ingestionRate = valueFactory.groupValueFor(OsmosePropertyName.PREDATION_INGESTION_RATE_MAX, group);
                        ...
                        LOG.log(Level.WARNING, getMsg(OsmosePropertyName.PREDATION_EFFICIENCY_CRITICAL), ex);
                    }
                    return value;
                };
            }
            ...
            private boolean isValidPredationEfficiency(float ingestionRateParsed, float maintQB) {
                ...
                if (ingestionRateParsed == 0) {
                    LOG.warning(getMsg(OsmosePropertyName.PREDATION_EFFICIENCY_CRITICAL) + ": tried to divide by zero value for [" + OsmosePropertyName.PREDATION_INGESTION_RATE_MAX + "]");
                } else {
                    if (maintQB >= ingestionRateParsed) {
                        LOG.warning(getMsg(OsmosePropertyName.PREDATION_EFFICIENCY_CRITICAL) + ": [" + OsmosePropertyName.PREDATION_INGESTION_RATE_MAX + ":" + ingestionRateParsed + "] may not be smaller than [" + FISHBASE_POPQB_MAINT_QB + ":" + maintQB + "].");
                    } else {
                        valid = true;
                    }
                }
