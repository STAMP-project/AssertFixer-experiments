...
public class Commands {
  ...
  public void rerun(String line, DispatchCallback callback) {
    ...
    if (cmd.length > 2 || (cmd.length == 2 && !cmd[1].matches("-?\\d+"))) {
      sqlLine.error("Usage: rerun <offset>, max offset is " + (size == 0
          ? 0 : "+/-" + (size - 1)));
      callback.setToFailure();
      return;
    }

    int offset = cmd.length == 1 ? 0 : Integer.parseInt(cmd[1]) - 1;

    if (size - 1 < Math.abs(offset)) {
      sqlLine.error("Usage: rerun <offset>, max offset is " + (size == 0
              ? 0 : "+/-" + (size - 1)));
      callback.setToFailure();
      return;
    }
    Iterator<History.Entry> iterator = offset > 0
        ? history.iterator(offset)
        : history.reverseIterator(size - 1 + offset);

    callback.setToSuccess();
    String command = iterator.next().line();
    sqlLine.runCommands(Collections.singletonList(command), callback);
  }
