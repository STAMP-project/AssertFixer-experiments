...
public class ProjectFactory {
    ...
    private Map<String, List<MetadataFieldGroup>> projectMetadataFieldGroups = new HashMap<String, List<MetadataFieldGroup>>();
    ...
    public List<MetadataFieldGroup> getProjectFields(String projectName) {
        ...
        List<MetadataFieldGroup> projectFields = projectMetadataFieldGroups.get(projectName);

        if (projectFields == null) {
            logger.info("No metadata field groups cached for " + projectName);
            projectFields = new ArrayList<MetadataFieldGroup>();

            Instant start = Instant.now();
            Project project = projectRepo.findByName(projectName);
            logger.info(Duration.between(start, Instant.now()).toMillis() + " milliseconds to lookup project");
            ...
            if (newProject) {
                start = Instant.now();
                project = createProject(projectName);
                logger.info(Duration.between(start, Instant.now()).toMillis() + " milliseconds to create new project");
            }

            start = Instant.now();
            final Iterable<JsonNode> nodesOfProject = () -> getProjectNode(projectName).get(METADATA_KEY).elements();
            logger.info(Duration.between(start, Instant.now()).toMillis() + " milliseconds to get project node");
            ...
            for (JsonNode metadata : nodesOfProject) {
                ...
                logger.info(Duration.between(start, Instant.now()).toMillis() + " milliseconds to look up field profile");
                if (fieldProfile == null) {
                    ...
                    logger.info(Duration.between(start, Instant.now()).toMillis() + " milliseconds to create new field profile");
                }
                ...
                logger.info(Duration.between(start, Instant.now()).toMillis() + " milliseconds to look up metadata field label");
                if (metadataFieldLabel == null) {
                    start = Instant.now();
                    metadataFieldLabel = metadataFieldLabelRepo.create(labelName, fieldProfile);
                    logger.info(Duration.between(start, Instant.now()).toMillis() + " milliseconds to create new metadata field label");
                }
            ...
            if (newProject) {
                ...
                logger.info(Duration.between(start, Instant.now()).toMillis() + " milliseconds to update new project");
            }

            projectMetadataFieldGroups.put(projectName, projectFields);
        }
    ...
    private List<ProjectRepository> getProjectRepositories(JsonNode projectNode) {
    ...
    private List<ProjectAuthority> getProjectAuthorities(JsonNode projectNode) {
    ...
    private List<ProjectSuggestor> getProjectSuggestors(JsonNode projectNode) {
    ...
    private JsonNode getProjectsNode() {
    ...
    private JsonNode readProjectsNode() {
        String json = null;
        try {
    ...
    private JsonNode getProjectNode(String projectName) {
        JsonNode profileNode = getProjectsNode().get(projectName);
        if (profileNode == null) {
            profileNode = getProjectsNode().get(DEFAULT_PROJECT_KEY);
        }
        return profileNode;
    }
