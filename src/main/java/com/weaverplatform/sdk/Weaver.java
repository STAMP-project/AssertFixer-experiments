...
public class Weaver {
  ...
  private Client getClientMultiPart() {
    if(clientMultiPart == null) {
      ...
      clientMultiPart = Client.create(cc);
    }
  ...
  public WeaverFile snapshotToStore(boolean zipped) {
    ...
    return converter.fromJson(soapResponse.getEntity(String.class), WeaverFile.class);
  }

  public InputStream getSnapshot(boolean zipped) {
    JsonObject payload = new JsonObject();
    payload.addProperty("json", false);
    payload.addProperty("zipped", zipped);
  ...
  public WeaverFile snapshotGraphToStore(List<String> graphs, List<String> toGraphs, List<String> fromGraphs, boolean zipped) {
    JsonObject payload = new JsonObject();
    if(graphs != null && !graphs.isEmpty()) {
      JsonElement array = converter.toJsonTree(graphs, new TypeToken<List<String>>() {}.getType());
      payload.add("graphs", array);
    }
    if(toGraphs != null && !toGraphs.isEmpty()) {
      JsonElement array = converter.toJsonTree(toGraphs, new TypeToken<List<String>>() {}.getType());
      payload.add("toGraphs", array);
    }
    if(fromGraphs != null && !fromGraphs.isEmpty()) {
      JsonElement array = converter.toJsonTree(fromGraphs, new TypeToken<List<String>>() {}.getType());
  ...
  public InputStream getSnapshotGraph(List<String> graphs, List<String> toGraphs, List<String> fromGraphs, boolean zipped) {
    JsonObject payload = new JsonObject();
    if(graphs != null && !graphs.isEmpty()) {
      JsonElement array = converter.toJsonTree(graphs, new TypeToken<List<String>>() {}.getType());
      payload.add("graphs", array);
