...
public class ChunkedWriter {
  ...
  private CreateAttributeOperation createAttributeCreationOperation(ResultSet rs) throws SQLException {
    ...
    CreateAttributeOperation operation;
    switch(resolvePrimitiveTypeOfResultSet(rs)) {
      case DATE:
        String dateValue = DateTimeParser.getUTCTime(rs.getTimestamp("datetime_value", UTCTimestamp.UTC_CALENDAR).getTime());
        operation = new CreateAttributeOperation(user, id, sourceId, key, dateValue, ad);
        break;
      case BOOLEAN:
        Boolean booleanValue = rs.getBoolean("boolean_value");
        operation = new CreateAttributeOperation(user, id, sourceId, key, booleanValue, ad);
        break;
      case STRING:
        String stringValue = rs.getString("string_value");
        operation = new CreateAttributeOperation(user, id, sourceId, key, stringValue, ad);
        break;
      case DOUBLE:
        Double doubleValue = rs.getDouble("double_value");
        operation = new CreateAttributeOperation(user, id, sourceId, key, doubleValue, ad);
        break;
      default:
        throw new RuntimeException("Could not create Attribute write operation");
    }
