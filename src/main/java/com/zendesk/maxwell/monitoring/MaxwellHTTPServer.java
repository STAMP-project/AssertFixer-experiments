...
import static com.zendesk.maxwell.monitoring.MaxwellMetrics.reportingTypePrometheus;
...
public class MaxwellHTTPServer {
	...
	public static void startIfRequired(MaxwellContext context) throws IOException {
		...
		if (metricsRegistries != null || diagnosticContext != null) {
			...
			String reportingType = context.getConfig().metricsReportingType;
			MaxwellHTTPServerWorker maxwellHTTPServerWorker = new MaxwellHTTPServerWorker(httpBindAddress, port, pathPrefix, metricsRegistries, diagnosticContext, reportingType);
	...
	private static MaxwellMetrics.Registries getMetricsRegistries(MaxwellContext context) throws IOException {
		...
		if (reportingType != null && (reportingType.contains(reportingTypeHttp) || reportingType.contains(reportingTypePrometheus))) {
...
class MaxwellHTTPServerWorker implements StoppableTask, Runnable {
	...
	private final String metricsReportingType;
	private Server server;

	public MaxwellHTTPServerWorker(String bindAddress, int port, String pathPrefix, MaxwellMetrics.Registries metricsRegistries, MaxwellDiagnosticContext diagnosticContext, String reportingType) {
		this.bindAddress = bindAddress;
		this.port = port;
		this.pathPrefix = pathPrefix;
		this.metricsRegistries = metricsRegistries;
		this.diagnosticContext = diagnosticContext;
		this.metricsReportingType = reportingType;
	}
	...
	public void startServer() throws Exception {
		...
		if (metricsRegistries != null) {
			...
			if (this.metricsReportingType.contains(reportingTypeHttp)) {
				handler.addServlet(new ServletHolder(new MetricsServlet(metricsRegistries.metricRegistry)), "/metrics");
			}

			if (this.metricsReportingType.contains(reportingTypePrometheus)) {
				handler.addServlet(new ServletHolder(new io.prometheus.client.exporter.MetricsServlet()), "/prometheus-metrics");
			}
