...
import com.helger.validator.servlet.AppWebAppListener;
...
public final class MainStart extends AbstractMain
{
  public static void main (final String [] args) throws Exception
  {
    ...
    if (bCanStart)
    {
      ...
      // file:/D:/git/phoss-validator/target/classes/WEB-INF/web.xml
      // jar:file:/D:/git/phoss-validator/target/phoss-validator-0.1.0-SNAPSHOT.jar!/WEB-INF/web.xml
      String sBaseURL = sWebXmlURL;
      if (DefaultResourceResolver.isExplicitJarFileResource (sBaseURL))
      {
        // Cmdline:
        // jar:file:/D:/git/phoss-validator/target/phoss-validator-0.1.0-SNAPSHOT.jar!/
        sBaseURL = sBaseURL.substring (0, sBaseURL.lastIndexOf ('!') + 1) + "/";
      }
      else
      {
        // In Eclipse:
        // file:/D:/git/phoss-validator/target/classes
        sBaseURL = StringHelper.trimEnd (sBaseURL, "WEB-INF/web.xml");
      }

      // Used in the WebAppListener of this project
      SystemProperties.setPropertyValue (AppWebAppListener.SYS_PROP_SERVLET_CONTEXT_BASE, sBaseURL);
    ...
    {
      new HelpFormatter ().printHelp ("phoss-validator start", aOptions, true);
