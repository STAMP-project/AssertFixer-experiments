...
public class LocalDashboard implements Dashboard {
    ...
    public static void setExecutedTests(int executedTests, int executedTestsWithVideo) {
        LocalDashboard.executedTests = executedTests;
        LocalDashboard.executedTestsWithVideo = executedTestsWithVideo;
    }
    ...
    public static void setShutDownHook() {
        if (!shutdownHookAdded.getAndSet(true)) {
            try {
                Runtime.getRuntime().addShutdownHook(new Thread(LocalDashboard::dumpTestInformationToFile, "Dashboard dumpTestInformationToFile shutdown hook"));
            } catch (Exception e) {
                LOGGER.warn(e.toString(), e);
            }
        }
    ...
    public static void setCommonProxyUtilities(CommonProxyUtilities commonProxyUtilities) {
        LocalDashboard.commonProxyUtilities = commonProxyUtilities;
    }
