ALTER TABLE nodes
  DROP CONSTRAINT nodes_deleted_by_fkey,
  ALTER COLUMN deleted_by SET DEFAULT -1;

DROP INDEX nodes_uid_graph_deleted_key;
CREATE UNIQUE INDEX nodes_uid_graph_deleted_key ON nodes (uid, graph, deleted_by);
...
CREATE OR REPLACE VIEW live_relations AS
 SELECT e.id,
  ...
  WHERE NOT (EXISTS ( SELECT 1
           FROM nodes r
          WHERE r.id = e.node AND r.deleted_by >= 0
...
CREATE OR REPLACE VIEW live_attributes AS
 SELECT attributes.id,
  ...
  WHERE NOT (EXISTS (SELECT 1
                     FROM nodes r
                     WHERE r.id = attributes.node AND r.deleted_by >= 0
                    ))
 ...
 SELECT row_number() OVER (ORDER BY individuals.id) AS seqnr,
    ...
    individuals.value
          ...
          WHERE true
            ...
            AND NOT (nodes.id IN ( SELECT removed_nodes.deleted_by FROM nodes removed_nodes WHERE deleted_by >= 0))
        ...
        UNION ALL
         SELECT rnv.meta_id AS id,
          ...
          WHERE true
            ...
            AND NOT (rnv.removed_node IN ( SELECT removed_nodes.deleted_by FROM nodes removed_nodes WHERE deleted_by >= 0))
...
BEGIN
  ...
  CREATE TEMPORARY TABLE to_delete ON COMMIT DROP AS SELECT id AS removed_node FROM nodes WHERE deleted_by >= 0;
  ...
  INSERT INTO to_delete SELECT deleted_by FROM nodes WHERE deleted_by >= 0;
