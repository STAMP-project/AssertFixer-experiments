...
BEGIN
  ...
  WITH new_node_ids AS (
    ...
    new_attributes AS (
    INSERT INTO attributes (node, source, key, double_value, string_value, boolean_value, datetime_value)
      SELECT (SELECT id FROM new_attribute_nodes WHERE new_uid = CONCAT(target_node_uid, '#', la.node)),
      nni.new_id,
      la.key,
      la.double_value,
      la.string_value,
