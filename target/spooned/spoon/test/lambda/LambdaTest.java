...
public class LambdaTest {
    ...
    @org.junit.Test
    public void testInterfaceWithObjectMethods() throws java.lang.Exception {
        spoon.reflect.declaration.CtInterface<?> checkPersons = factory.Interface().get(spoon.test.lambda.testclasses.Foo.CheckPersons.class);
        java.util.List<spoon.reflect.code.CtLambda<?>> lambdas = foo.filterChildren(new spoon.reflect.visitor.filter.LambdaFilter(checkPersons)).list();
        org.junit.Assert.assertEquals(2, lambdas.size());
        spoon.reflect.code.CtLambda<?> lambda = lambdas.get(0);
        org.junit.Assert.assertEquals(2, lambda.getParameters().size());
        spoon.reflect.declaration.CtMethod<?> method = lambda.getOverriddenMethod();
        org.junit.Assert.assertTrue(checkPersons.getMethods().contains(method));
        org.junit.Assert.assertEquals("test", method.getSimpleName());
    }
