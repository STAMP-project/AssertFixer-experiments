...
public class PatternTest {
    ...
    @org.junit.Test
    public void testGenerateIfElse() throws java.lang.Exception {
        spoon.reflect.declaration.CtType<?> type = spoon.testing.utils.ModelUtils.buildClass(spoon.test.template.testclasses.match.GenerateIfElse.class);
        spoon.pattern.Pattern pattern = spoon.pattern.PatternBuilder.create(new spoon.pattern.PatternBuilderHelper(type).setBodyOfMethod("generator").getPatternElements()).configurePatternParameters(( pb) -> {
            pb.parameter("option").byVariable("option");
        }).configureInlineStatements(( lsb) -> lsb.inlineIfOrForeachReferringTo("option")).configurePatternParameters(( pb) -> {
            pb.parameter("value").byFilter(new spoon.reflect.visitor.filter.TypeFilter(spoon.reflect.code.CtLiteral.class));
        }).build();
        {
            java.util.List<spoon.reflect.code.CtStatement> statements = pattern.generator().generate(spoon.reflect.code.CtStatement.class, new spoon.support.util.ImmutableMapImpl().putValue("option", true).putValue("value", "spoon"));
            org.junit.Assert.assertEquals(1, statements.size());
            org.junit.Assert.assertEquals("java.lang.System.out.print(\"spoon\")", statements.get(0).toString());
        }
        {
            java.util.List<spoon.reflect.code.CtStatement> statements = pattern.generator().generate(spoon.reflect.code.CtStatement.class, new spoon.support.util.ImmutableMapImpl().putValue("option", false).putValue("value", 2.1));
            org.junit.Assert.assertEquals(1, statements.size());
            org.junit.Assert.assertEquals("java.lang.System.out.println(2.1)", statements.get(0).toString());
        }
    }
