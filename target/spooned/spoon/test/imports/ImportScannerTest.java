...
public class ImportScannerTest {
    ...
    public void testImportOnSpoon() throws java.io.IOException {
        ...
        if ((!(missingImports.isEmpty())) || (!(unusedImports.isEmpty()))) {
            ...
            java.lang.System.err.println((((("Import scanner imports " + countUnusedImports) + " unused imports and misses ") + countMissingImports) + " imports"));
            ...
            for (CtType type : keys) {
                java.lang.System.err.println(type.getQualifiedName());
                if (missingImports.containsKey(type)) {
                    List<String> imports = missingImports.get(type);
                    for (String anImport : imports) {
                        java.lang.System.err.println((("\t" + anImport) + " missing"));
                    }
                }
                if (unusedImports.containsKey(type)) {
                    List<String> imports = unusedImports.get(type);
                    for (String anImport : imports) {
                        java.lang.System.err.println((("\t" + anImport) + " unused"));
                    }
                }
            }
            org.junit.Assert.assertEquals((((("Import scanner imports " + countUnusedImports) + " unused imports and misses ") + countMissingImports) + " imports"), 0, (countMissingImports + countUnusedImports));
        }
    }
