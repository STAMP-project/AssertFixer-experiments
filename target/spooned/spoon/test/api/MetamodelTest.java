    ...
    public void spoonMetaModelTest() {
        ...
        mm.getConcepts().forEach(( mmConcept) -> {
            mmConcept.getRoleToProperty().forEach(( role, mmField) -> {
                ...
                mmField.getMethods(spoon.metamodel.MMMethodKind.OTHER).forEach(( mmethod) -> mmethod.getRelatedMethods().forEach(( ctMethod) -> problems.add(((("Unhandled method signature: " + (ctMethod.getDeclaringType().getSimpleName())) + "#") + (ctMethod.getSignature())))));
            });
        });
