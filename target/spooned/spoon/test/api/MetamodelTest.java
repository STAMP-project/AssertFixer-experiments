...
public class MetamodelTest {
    ...
    public void testGetAllMetamodelInterfacess() {
        ...
        org.hamcrest.MatcherAssert.assertThat(spoon.metamodel.Metamodel.getAllMetamodelInterfaces().stream().map(( x) -> x.getQualifiedName()).collect(java.util.stream.Collectors.toSet()), org.hamcrest.core.IsEqual.equalTo(interfaces.getModel().getAllTypes().stream().map(( x) -> x.getQualifiedName()).collect(java.util.stream.Collectors.toSet())));
    }

    @org.junit.Test
    public void testRuntimeMetamodel() {
        spoon.metamodel.Metamodel testMetaModel = spoon.metamodel.Metamodel.getInstance();
        java.util.Map<java.lang.String, spoon.metamodel.MetamodelConcept> expectedTypesByName = new java.util.HashMap<>();
        testMetaModel.getConcepts().forEach(( t) -> {
            if ((t.getKind()) == (spoon.metamodel.ConceptKind.LEAF)) {
        ...
        for (spoon.test.api.Metamodel.Type type : spoon.test.api.Metamodel.getAllMetamodelTypes()) {
            spoon.metamodel.MetamodelConcept expectedType = expectedTypesByName.remove(type.getName());
            org.junit.Assert.assertSame(expectedType.getImplementationClass().getActualClass(), type.getModelClass());
            org.junit.Assert.assertSame(expectedType.getMetamodelInterface().getActualClass(), type.getModelInterface());
            java.util.Map<spoon.reflect.path.CtRole, spoon.metamodel.MetamodelProperty> expectedRoleToField = new java.util.HashMap<>(expectedType.getRoleToProperty());
            for (spoon.test.api.Metamodel.Field field : type.getFields()) {
                spoon.metamodel.MetamodelProperty expectedField = expectedRoleToField.remove(field.getRole());
        ...
        org.junit.Assert.assertTrue(java.lang.String.join("\n", problems), problems.isEmpty());
    }
    ...
    public void testMetamodelWithoutSources() {
        spoon.metamodel.Metamodel runtimeMM = spoon.metamodel.Metamodel.getInstance();
        java.util.Collection<spoon.metamodel.MetamodelConcept> concepts = runtimeMM.getConcepts();
        spoon.metamodel.Metamodel sourceBasedMM = new spoon.metamodel.Metamodel(new java.io.File("src/main/java"));
    ...
    private void assertConceptsEqual(spoon.metamodel.MetamodelConcept expectedConcept, spoon.metamodel.MetamodelConcept runtimeConcept) {
        ...
        if ((expectedConcept.getImplementationClass()) == null) {
            org.junit.Assert.assertNull(runtimeConcept.getImplementationClass());
        }else {
            org.junit.Assert.assertNotNull(runtimeConcept.getImplementationClass());
            org.junit.Assert.assertEquals(expectedConcept.getImplementationClass().getActualClass(), runtimeConcept.getImplementationClass().getActualClass());
        }
        org.junit.Assert.assertEquals(expectedConcept.getMetamodelInterface().getActualClass(), runtimeConcept.getMetamodelInterface().getActualClass());
    ...
    private void assertPropertiesEqual(spoon.metamodel.MetamodelProperty expectedProperty, spoon.metamodel.MetamodelProperty runtimeProperty) {
        ...
        org.junit.Assert.assertEquals(expectedProperty.getTypeofItems().getActualClass(), runtimeProperty.getTypeofItems().getActualClass());
        org.junit.Assert.assertEquals(expectedProperty.getOwner().getName(), runtimeProperty.getOwner().getName());
        org.junit.Assert.assertSame(expectedProperty.getContainerKind(), runtimeProperty.getContainerKind());
        org.junit.Assert.assertEquals(expectedProperty.getTypeOfField(), runtimeProperty.getTypeOfField());
    ...
    public void testMetamodelCachedInFactory() throws java.io.IOException {
        spoon.metamodel.Metamodel.getInstance().getConcepts();
    }
    ...
    public void spoonMetaModelTest() {
        ...
        spoon.metamodel.Metamodel mm = spoon.metamodel.Metamodel.getInstance();
        java.util.List<java.lang.String> problems = new java.util.ArrayList<>();
        ...
        mm.getConcepts().forEach(( mmConcept) -> {
            mmConcept.getRoleToProperty().forEach(( role, mmField) -> {
                ...
                if ((mmField.getMethod(spoon.metamodel.MMMethodKind.SET)) == null) {
                    if (((new spoon.support.visitor.ClassTypingContext(mmConcept.getMetamodelInterface()).isSubtypeOf(factory.Type().createReference(spoon.reflect.reference.CtReference.class))) == false) && ((mmConcept.getName().equals("CtTypeInformation")) == false)) {
                        problems.add(((("Missing setter for " + (mmField.getOwner().getName())) + " and CtRole.") + (mmField.getRole())));
                    }
                }
                org.junit.Assert.assertFalse((("Value type of Field " + (mmField.toString())) + " is implicit"), mmField.getTypeOfField().isImplicit());
                org.junit.Assert.assertFalse((("Item value type of Field " + (mmField.toString())) + " is implicit"), mmField.getTypeofItems().isImplicit());
                mmField.getMethods(spoon.metamodel.MMMethodKind.OTHER).forEach(( mmethod) -> mmethod.getDeclaredMethods().forEach(( ctMethod) -> problems.add(((("Unhandled method signature: " + (ctMethod.getDeclaringType().getSimpleName())) + "#") + (ctMethod.getSignature())))));
            });
        });
    ...
    public void testGetRoleHandlersOfClass() {
        int countOfIfaces = 0;
        for (spoon.reflect.declaration.CtType spoonIface : spoon.metamodel.Metamodel.getAllMetamodelInterfaces()) {
            countOfIfaces++;
