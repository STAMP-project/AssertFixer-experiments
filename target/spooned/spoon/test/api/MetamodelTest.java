    ...
    public void spoonMetaModelTest() {
        ...
        mm.getConcepts().forEach(( mmConcept) -> {
            mmConcept.getRoleToProperty().forEach(( role, mmField) -> {
                ...
                mmField.getMethods(spoon.metamodel.MMMethodKind.OTHER).forEach(( mmethod) -> problems.add(((("Unhandled method signature: " + (mmethod.getActualCtMethod().getDeclaringType().getSimpleName())) + "#") + (mmethod.getActualCtMethod().getSignature()))));
            });
        });
