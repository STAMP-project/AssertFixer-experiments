...
public class JDTBasedSpoonCompiler implements spoon.SpoonModelBuilder {
    ...
    protected void buildModel(org.eclipse.jdt.internal.compiler.ast.CompilationUnitDeclaration[] units) {
        if ((getEnvironment().getSpoonProgress()) != null) {
            getEnvironment().getSpoonProgress().start(spoon.support.compiler.SpoonProgress.STEP.MODEL);
        }
        ...
        unitLoop : for (org.eclipse.jdt.internal.compiler.ast.CompilationUnitDeclaration unit : unitList) {
            if ((unit.isModuleInfo()) || (!(unit.isEmpty()))) {
                ...
                if ((getEnvironment().getSpoonProgress()) != null) {
                    getEnvironment().getSpoonProgress().step(spoon.support.compiler.SpoonProgress.STEP.MODEL, new java.lang.String(unit.getFileName()), (++i), unitList.size());
                }
            }
        }
        if ((getEnvironment().getSpoonProgress()) != null) {
            getEnvironment().getSpoonProgress().end(spoon.support.compiler.SpoonProgress.STEP.MODEL);
        }
        if (getFactory().getEnvironment().isAutoImports()) {
            if ((getEnvironment().getSpoonProgress()) != null) {
                getEnvironment().getSpoonProgress().start(spoon.support.compiler.SpoonProgress.STEP.IMPORT);
            }
            i = 0;
            for (org.eclipse.jdt.internal.compiler.ast.CompilationUnitDeclaration unit : units) {
                ...
                if ((getEnvironment().getSpoonProgress()) != null) {
                    getEnvironment().getSpoonProgress().step(spoon.support.compiler.SpoonProgress.STEP.IMPORT, new java.lang.String(unit.getFileName()), (++i), units.length);
                }
            }
            if ((getEnvironment().getSpoonProgress()) != null) {
                getEnvironment().getSpoonProgress().end(spoon.support.compiler.SpoonProgress.STEP.IMPORT);
            }
        }
    }
