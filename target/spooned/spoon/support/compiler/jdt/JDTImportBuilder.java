...
class JDTImportBuilder {
    ...
    private spoon.reflect.declaration.CtType getOrLoadClass(java.lang.String className) {
        spoon.reflect.declaration.CtType klass = this.factory.Type().get(className);
        if (klass == null) {
            ...
            if (klass == null) {
                try {
                    ...
                    if ((!(className.contains(spoon.reflect.declaration.CtType.INNERTTYPE_SEPARATOR))) && (className.contains(spoon.reflect.declaration.CtPackage.PACKAGE_SEPARATOR))) {
                        int lastIndexOfDot = className.lastIndexOf(spoon.reflect.declaration.CtPackage.PACKAGE_SEPARATOR);
                        java.lang.String classNameWithInnerSep = ((className.substring(0, lastIndexOfDot)) + (spoon.reflect.declaration.CtType.INNERTTYPE_SEPARATOR)) + (className.substring((lastIndexOfDot + 1)));
