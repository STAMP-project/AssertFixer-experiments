...
public class QueueProcessingManager implements spoon.processing.ProcessingManager {
    ...
    public void process(java.util.Collection<? extends spoon.reflect.declaration.CtElement> elements) {
        ...
        if ((factory.getEnvironment().getSpoonProgress()) != null) {
            factory.getEnvironment().getSpoonProgress().start(spoon.support.compiler.SpoonProgress.STEP.PROCESS);
        }
        int i = 0;
        while ((p = processors.poll()) != null) {
            try {
                ...
                p.processingDone();
                if ((factory.getEnvironment().getSpoonProgress()) != null) {
                    factory.getEnvironment().getSpoonProgress().step(spoon.support.compiler.SpoonProgress.STEP.PROCESS, p.getClass().getName(), (++i), getProcessors().size());
                }
            }
        } 
        if ((factory.getEnvironment().getSpoonProgress()) != null) {
            factory.getEnvironment().getSpoonProgress().end(spoon.support.compiler.SpoonProgress.STEP.PROCESS);
        }
    }
