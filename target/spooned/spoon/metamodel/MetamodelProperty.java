...
public class MetamodelProperty {
    ...
    public boolean isDerived() {
        if ((derived) == null) {
            ...
            for (spoon.reflect.declaration.CtMethod<?> ctMethod : getter.getRelatedMethods()) {
    ...
    public boolean isUnsettable() {
        if ((unsettable) == null) {
            ...
            boolean isConreteMethod = false;
            for (spoon.reflect.declaration.CtMethod<?> ctMethod : setter.getRelatedMethods()) {
    ...
    public spoon.metamodel.MetamodelProperty getRootSuperField() {
        ...
        superProperties.forEach(( superField) -> {
            spoon.metamodel.Metamodel.addUniqueObject(potentialRootSuperFields, superField.getRootSuperField());
        });
        int idx = 0;
