...
public class OpenWireProtocolManager implements ProtocolManager<Interceptor>, ClusterTopologyListener {
   ...
   public OpenWireProtocolManager(OpenWireProtocolManagerFactory factory, ActiveMQServer server) {
      ...
      if (supportAdvisory) {
         ...
         clusterManager.removeFromIgnoredAddressPrefixes(AdvisorySupport.ADVISORY_TOPIC_PREFIX);
      }
   }
   ...
   public void setSupportAdvisory(boolean supportAdvisory) {
      final boolean oldValue = this.supportAdvisory;
      if (oldValue != supportAdvisory) {
         if (supportAdvisory) {
            this.server.getClusterManager().addToIgnoredAddressPrefixes(AdvisorySupport.ADVISORY_TOPIC_PREFIX);
         } else {
            this.server.getClusterManager().removeFromIgnoredAddressPrefixes(AdvisorySupport.ADVISORY_TOPIC_PREFIX);
         }
