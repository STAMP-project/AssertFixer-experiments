...
public class ProtonServerSenderContext extends ProtonInitializable implements ProtonDeliveryHandler {
   ...
   /* This is to be captured during the delivery, within the worker thread.
      When a flow control event happens we will get the delta and perform the credit calculation.
      This is because at the time I wrote this (proton 0.29) there was no exposure of the flow control frame
      so I had to calculate backwards what was the delta.
      Notice that ServerConsumer has its own counter, which may be ahead of what has been scheduled for delivery on proton executor.
      With this we can pace injection into proton properly.
    */
   private int lastCredit = 0;
   ...
   public void onFlow(int currentCredits, boolean drain) {
      ...
      int delta = sender.getCredit() - lastCredit;

      ServerConsumerImpl serverConsumer = (ServerConsumerImpl) brokerConsumer;
      if (drain) {
         // receiveCredits(0) will stop anything from being delivered
         lastCredit = 0;
         serverConsumer.receiveCredits(0);
         ...
         if (draining.compareAndSet(false, true)) {
            ...
            serverConsumer.forceDelivery(1, new Runnable() {
               ...
               public void run() {
                  try {
                     connection.runLater(() -> plugSender.reportDrained());
                  } finally {
                     draining.set(false);
                  }
               }
         ...
         serverConsumer.receiveCredits(delta);
      }
   }
      ...
      try {
         connection.runNow(() -> {
            ...
            try {
               ...
               // See note on lastCredit declaration
               lastCredit = sender.getCredit();
