...
public class ProtonServerSenderContext extends ProtonInitializable implements ProtonDeliveryHandler {
   ...
   private static final int PACE_AT = 100;
   private static final int RESUME_AT = 10;
   ...
   public boolean hasCredits() {
      if (!connection.flowControl(brokerConsumer::promptDelivery)) {
         System.out.println("Netty is busy");
         return false;
      }
   ...
   public void onMessage(Delivery delivery) throws ActiveMQAMQPException {
      ...
      connection.runOnPool(() -> internalonMessage(delivery, remotestate));
   }

   private void internalonMessage(Delivery delivery, DeliveryState remoteState) {
      if (closed) {
         return;
      }
      try {
         ...
         try {
            ...
            if (settleImmediate) {
               connection.runLater(() -> delivery.settle());
            }
   ...
   public int deliverMessage(MessageReference messageReference, int deliveryCount, Connection transportConnection) throws Exception {
      ...
      try {
         int currentSize;
         synchronized (creditsLock) {
            currentSize = pending.incrementAndGet();
            if (currentSize > PACE_AT) {
               paced = true;
            }
            credits--;
         }
         connection.runLater(() -> {
            ...
            try {
               ...
               synchronized (creditsLock) {
                  int result = pending.decrementAndGet();
                  if (paced && result < RESUME_AT) {
                     paced = false;
