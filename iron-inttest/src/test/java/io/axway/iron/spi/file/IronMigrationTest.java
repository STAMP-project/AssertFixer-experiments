...
public class IronMigrationTest {
    ...
    private Stream<Path> computeExpectedPaths(int lastTenantIdx, Path destIronPath, HashMap<String, Set<String>> snapshotIdsByStoreType) {
        ...
        expectedPaths.add(destIronPath.resolve("global").resolve("tx").resolve("00000000000000000000_global.tx"));
        // Add tenant paths
        for (String usedSourceFileName : snapshotIdsByStoreType.getOrDefault("tenant", emptySet())) {
            ...
            IntStream.range(0, lastTenantIdx + 1).boxed().map(n -> tenantSnapshot.resolve(n + ".snapshot")).forEach(path -> expectedPaths.add(path));
        }
        Optional<String> lastSnapshotId = snapshotIdsByStoreType.getOrDefault("tenant", emptySet()).stream().max(String::compareTo);
        expectedPaths.add(destIronPath.resolve("tenant").resolve("tx").resolve(lastSnapshotId.get() + "_0.tx"));
