...
public class FileStoreTest {
    ...
    @Test(expectedExceptions = RuntimeException.class, expectedExceptionsMessageRegExp = "Iron database is corrupted.*")
    public void shouldIronNotStartWithALackingTransaction() throws IOException, ExecutionException, InterruptedException {
        // Given an iron store with a snapshot
        Path filePath = Paths.get("tmp-iron-test", "iron-spi-file-inttest");
        ...
        // Corrupt the Iron database by removing the last transaction
        Files.delete(filePath.resolve(directory).resolve("tx").resolve("00000000000000000001_store2.tx"));

        // When I try to open the store after removing the last transaction
        try (StoreManager storeManagerAfterRemovingLastTransaction = factoryBuilder.build()) {
            // nothing to do
        }
    }
}
