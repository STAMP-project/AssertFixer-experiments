...
class PostgresEventStoreIT {
    ...
    fun `Can roundtrip events`() {
        runBlocking {
            ...
            Assertions.assertThat(data).containsExactly(UntypedEvent(metadata, mapOf("foo" to "1")), UntypedEvent(metadata, mapOf("foo" to "2")))

            val (data2, _) = eventStore.get(EventQuery(cursor = cursor))
            Assertions.assertThat(data2).containsExactly(UntypedEvent(metadata, mapOf("foo" to "3")))
        }
    }
}
