 ...
 * @since 3.2.
 */
public class UnmodifiableByteObjectMap<V>
        implements MutableByteObjectMap<V>, Serializable
{
    private static final long serialVersionUID = 1L;
    private final MutableByteObjectMap<V> map;

    public UnmodifiableByteObjectMap(MutableByteObjectMap<V> map)
    {
        if (map == null)
        {
            throw new IllegalArgumentException("Cannot create a UnmodifiableByteObjectMap on a null map");
        }

        this.map = map;
    }

    private boolean isAbsent(V result, byte key)
    {
        return result == null && !this.containsKey(key);
    }

    private V getIfAbsentThrow(byte key)
    {
    ...
    @Override
    public V putPair(ByteObjectPair<V> keyValuePair)
    {
    ...
    @Override
    public MutableByteObjectMap<V> tap(Procedure<? super V> procedure)
    {
