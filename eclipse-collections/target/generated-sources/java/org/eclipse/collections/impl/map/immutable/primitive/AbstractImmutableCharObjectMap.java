...
import org.eclipse.collections.impl.block.procedure.checked.primitive.CheckedCharObjectProcedure;
 ...
 * @since 4.0.
 */
public abstract class AbstractImmutableCharObjectMap<V> implements ImmutableCharObjectMap<V>
{
    protected static class ImmutableCharObjectMapSerializationProxy<V> implements Externalizable
    {
        private static final long serialVersionUID = 1L;

        private CharObjectMap<V> map;

        @SuppressWarnings("UnusedDeclaration")
        public ImmutableCharObjectMapSerializationProxy()
        {
            // Empty constructor for Externalizable class
        }

        protected ImmutableCharObjectMapSerializationProxy(CharObjectMap<V> map)
        {
            this.map = map;
        }
        ...
        public void writeExternal(final ObjectOutput out) throws IOException
        {
            ...
            try
            {
                this.map.forEachKeyValue(new CheckedCharObjectProcedure<V>()
                {
                    @Override
                    public void safeValue(char key, V value) throws IOException
                    {
                        out.writeChar(key);
        ...
        {
            int size = in.readInt();
            MutableCharObjectMap<V> deserializedMap = new CharObjectHashMap<>();

            for (int i = 0; i < size; i++)
            {
                deserializedMap.put(in.readChar(), (V) in.readObject());
            }
