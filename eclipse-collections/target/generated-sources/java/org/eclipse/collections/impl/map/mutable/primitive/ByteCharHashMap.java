...
public class ByteCharHashMap extends AbstractMutableCharValuesMap implements MutableByteCharMap, Externalizable, MutableByteKeysMap
{
    ...
    {
        if (map instanceof ByteCharHashMap && ((ByteCharHashMap) map).occupiedWithSentinels == 0)
        {
            ByteCharHashMap hashMap = (ByteCharHashMap) map;
    ...
    private void addKeyValueAtIndex(byte key, char value, int index)
    {
    ...
    {
        return new UnmodifiableByteCharMap(this);
    }
    ...
    {
        return ByteCharMaps.immutable.ofAll(this);
    }
    ...
    private char getForSentinel(byte key, char ifAbsent)
    {
        if (isEmptyKey(key))
        {
            if (this.sentinelValues == null || !this.sentinelValues.containsZeroKey)
    ...
    private char slowGetIfAbsent(byte key, char ifAbsent)
    {
        int index = this.probe(key);
        if (this.keys[index] == key)
        {
    ...
    {
        ByteCharHashMap result = new ByteCharHashMap();
    ...
    public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException
    {
        ...
        {
            this.put(in.readByte(), in.readChar());
        }
    }
    ...
    private class InternalCharIterator implements MutableCharIterator
    {
        ...
        {
            return this.count < ByteCharHashMap.this.size();
        }
        ...
        public char next()
        {
            ...
            if (!this.handledZero)
            {
                ...
                if (ByteCharHashMap.this.containsKey(EMPTY_KEY))
                {
                    this.lastKey = EMPTY_KEY;
                    return ByteCharHashMap.this.get(EMPTY_KEY);
                }
            }
            if (!this.handledOne)
            {
                this.handledOne = true;
                if (ByteCharHashMap.this.containsKey(REMOVED_KEY))
                {
                    this.lastKey = REMOVED_KEY;
                    return ByteCharHashMap.this.get(REMOVED_KEY);
                }
            }
            byte[] keys = ByteCharHashMap.this.keys;
            while (!isNonSentinel(keys[this.position]))
            {
                this.position++;
            }
            this.lastKey = keys[this.position];
            char result = ByteCharHashMap.this.values[this.position];
            this.position++;
        ...
        public void remove()
        {
            ...
            ByteCharHashMap.this.removeKey(this.lastKey);
            this.count--;
    ...
    private class KeysView extends AbstractLazyByteIterable
    {
        ...
        {
            ByteCharHashMap.this.forEachKey(procedure);
        }
    }
    ...
    private class KeySet extends AbstractMutableByteKeySet
    {
        ...
        {
            return ByteCharHashMap.this;
        }

        @Override
        protected SentinelValues getSentinelValues()
        {
            return ByteCharHashMap.this.sentinelValues;
        }

        @Override
        protected byte getKeyAtIndex(int index)
        {
            return ByteCharHashMap.this.keys[index];
        }

        @Override
        protected int getTableSize()
        {
            return ByteCharHashMap.this.keys.length;
        }
        ...
        public boolean retainAll(ByteIterable source)
        {
            ...
            ByteCharHashMap retained = ByteCharHashMap.this.select((byte key, char value) -> sourceSet.contains(key));
            if (retained.size() != oldSize)
            {
                ByteCharHashMap.this.keys = retained.keys;
                ByteCharHashMap.this.values = retained.values;
                ByteCharHashMap.this.sentinelValues = retained.sentinelValues;
                ByteCharHashMap.this.occupiedWithData = retained.occupiedWithData;
                ByteCharHashMap.this.occupiedWithSentinels = retained.occupiedWithSentinels;
                return true;
            }
            return false;
        }
        ...
        {
            ByteCharHashMap.this.copyKeysOnWrite = true;
            boolean containsZeroKey = false;
            boolean containsOneKey = false;
            if (ByteCharHashMap.this.sentinelValues != null)
            {
                containsZeroKey = ByteCharHashMap.this.sentinelValues.containsZeroKey;
                containsOneKey = ByteCharHashMap.this.sentinelValues.containsOneKey;
            }
            return new ImmutableByteMapKeySet(ByteCharHashMap.this.keys, ByteCharHashMap.this.occupiedWithData, containsZeroKey, containsOneKey);
        }
    ...
    private class ValuesCollection extends AbstractCharValuesCollection
    {
        ...
        public MutableCharIterator charIterator()
        {
            return ByteCharHashMap.this.charIterator();
        }
        ...
        public boolean remove(char item)
        {
            ...
            if (ByteCharHashMap.this.sentinelValues != null)
            {
                if (ByteCharHashMap.this.sentinelValues.containsZeroKey && item == ByteCharHashMap.this.sentinelValues.zeroValue)
                {
                    ByteCharHashMap.this.removeKey(EMPTY_KEY);
                }
                if (ByteCharHashMap.this.sentinelValues.containsOneKey && item == ByteCharHashMap.this.sentinelValues.oneValue)
                {
                    ByteCharHashMap.this.removeKey(REMOVED_KEY);
                }
            }
            for (int i = 0; i < ByteCharHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ByteCharHashMap.this.keys[i]) && item == ByteCharHashMap.this.values[i])
                {
                    ByteCharHashMap.this.removeKey(ByteCharHashMap.this.keys[i]);
                }
            }
        ...
        public boolean retainAll(CharIterable source)
        {
            ...
            ByteCharHashMap retained = ByteCharHashMap.this.select((byte key, char value) -> sourceSet.contains(value));
            if (retained.size() != oldSize)
            {
                ByteCharHashMap.this.keys = retained.keys;
                ByteCharHashMap.this.values = retained.values;
    ...
    private class KeyValuesView extends AbstractLazyIterable<ByteCharPair>
    {
        @Override
        public void each(Procedure<? super ByteCharPair> procedure)
        {
            if (ByteCharHashMap.this.sentinelValues != null)
            {
                if (ByteCharHashMap.this.sentinelValues.containsZeroKey)
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, ByteCharHashMap.this.sentinelValues.zeroValue));
                }
                if (ByteCharHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, ByteCharHashMap.this.sentinelValues.oneValue));
                }
            }
            for (int i = 0; i < ByteCharHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ByteCharHashMap.this.keys[i]))
                {
                    procedure.value(PrimitiveTuples.pair(ByteCharHashMap.this.keys[i], ByteCharHashMap.this.values[i]));
                }
            }
        }

        @Override
        public void forEachWithIndex(ObjectIntProcedure<? super ByteCharPair> objectIntProcedure)
        {
            int index = 0;
            if (ByteCharHashMap.this.sentinelValues != null)
            {
                if (ByteCharHashMap.this.sentinelValues.containsZeroKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(EMPTY_KEY, ByteCharHashMap.this.sentinelValues.zeroValue), index);
                    index++;
                }
                if (ByteCharHashMap.this.sentinelValues.containsOneKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(REMOVED_KEY, ByteCharHashMap.this.sentinelValues.oneValue), index);
                    index++;
                }
            }
            for (int i = 0; i < ByteCharHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ByteCharHashMap.this.keys[i]))
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(ByteCharHashMap.this.keys[i], ByteCharHashMap.this.values[i]), index);
                    index++;
                }
            }
        }
        ...
        @Override
        public <P> void forEachWith(Procedure2<? super ByteCharPair, ? super P> procedure, P parameter)
        {
            if (ByteCharHashMap.this.sentinelValues != null)
            {
                if (ByteCharHashMap.this.sentinelValues.containsZeroKey)
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, ByteCharHashMap.this.sentinelValues.zeroValue), parameter);
                }
                if (ByteCharHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, ByteCharHashMap.this.sentinelValues.oneValue), parameter);
                }
            }
            for (int i = 0; i < ByteCharHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ByteCharHashMap.this.keys[i]))
                {
                    procedure.value(PrimitiveTuples.pair(ByteCharHashMap.this.keys[i], ByteCharHashMap.this.values[i]), parameter);
                }
            }
        }

        @Override
        public Iterator<ByteCharPair> iterator()
        {
            return new InternalKeyValuesIterator();
        }

        public class InternalKeyValuesIterator implements Iterator<ByteCharPair>
        {
            private int count;
            private int position;
            private boolean handledZero;
            ...
            public ByteCharPair next()
            {
                ...
                if (!this.handledZero)
                {
                    ...
                    if (ByteCharHashMap.this.containsKey(EMPTY_KEY))
                    {
                        return PrimitiveTuples.pair(EMPTY_KEY, ByteCharHashMap.this.sentinelValues.zeroValue);
                    }
                }
                if (!this.handledOne)
                {
                    ...
                    if (ByteCharHashMap.this.containsKey(REMOVED_KEY))
                    {
                        return PrimitiveTuples.pair(REMOVED_KEY, ByteCharHashMap.this.sentinelValues.oneValue);
                    }
                }
                ...
                ByteCharPair result = PrimitiveTuples.pair(keys[this.position], ByteCharHashMap.this.values[this.position]);
                this.position++;
                return result;
            }
