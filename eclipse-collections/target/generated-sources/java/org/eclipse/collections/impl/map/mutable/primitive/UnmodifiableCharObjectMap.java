 ...
 * @since 3.2.
 */
public class UnmodifiableCharObjectMap<V>
        implements MutableCharObjectMap<V>, Serializable
{
    private static final long serialVersionUID = 1L;
    private final MutableCharObjectMap<V> map;

    public UnmodifiableCharObjectMap(MutableCharObjectMap<V> map)
    {
        if (map == null)
        {
            throw new IllegalArgumentException("Cannot create a UnmodifiableCharObjectMap on a null map");
        }

        this.map = map;
    }

    private boolean isAbsent(V result, char key)
    {
        return result == null && !this.containsKey(key);
    }

    private V getIfAbsentThrow(char key)
    {
