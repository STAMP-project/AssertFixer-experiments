...
public class CharIntHashMap extends AbstractMutableIntValuesMap implements MutableCharIntMap, Externalizable, MutableCharKeysMap
{
    ...
    private int getForSentinel(char key, int ifAbsent)
    {
        if (isEmptyKey(key))
        {
            if (this.sentinelValues == null || !this.sentinelValues.containsZeroKey)
    ...
    private int slowGetIfAbsent(char key, int ifAbsent)
    {
        int index = this.probe(key);
        if (this.keys[index] == key)
        {
    ...
    private int fastGetIfAbsent(char key, int ifAbsent)
    {
    ...
    private int slowGetIfAbsentTwo(char key, int ifAbsent)
    {
    ...
    {
        MutableIntCharMap result = IntCharMaps.mutable.empty();
    ...
    private class InternalIntIterator implements MutableIntIterator
    {
        ...
        public int next()
        {
            ...
            if (!this.handledZero)
            {
                ...
                if (CharIntHashMap.this.containsKey(EMPTY_KEY))
                {
                    this.lastKey = EMPTY_KEY;
                    return CharIntHashMap.this.get(EMPTY_KEY);
                }
            }
            if (!this.handledOne)
            {
                this.handledOne = true;
                if (CharIntHashMap.this.containsKey(REMOVED_KEY))
                {
                    this.lastKey = REMOVED_KEY;
                    return CharIntHashMap.this.get(REMOVED_KEY);
                }
            }
    ...
    private class KeySet extends AbstractMutableCharKeySet
    {
        ...
        public boolean retainAll(CharIterable source)
        {
            ...
            CharIntHashMap retained = CharIntHashMap.this.select((char key, int value) -> sourceSet.contains(key));
            if (retained.size() != oldSize)
            {
                CharIntHashMap.this.keys = retained.keys;
                CharIntHashMap.this.values = retained.values;
        ...
        public CharSet freeze()
        {
            ...
            {
                containsZeroKey = CharIntHashMap.this.sentinelValues.containsZeroKey;
                containsOneKey = CharIntHashMap.this.sentinelValues.containsOneKey;
            }
    ...
    private class ValuesCollection extends AbstractIntValuesCollection
    {
        ...
        public boolean remove(int item)
        {
            ...
            if (CharIntHashMap.this.sentinelValues != null)
            {
                if (CharIntHashMap.this.sentinelValues.containsZeroKey && item == CharIntHashMap.this.sentinelValues.zeroValue)
                {
                    CharIntHashMap.this.removeKey(EMPTY_KEY);
                }
                if (CharIntHashMap.this.sentinelValues.containsOneKey && item == CharIntHashMap.this.sentinelValues.oneValue)
                {
                    CharIntHashMap.this.removeKey(REMOVED_KEY);
                }
            }
            for (int i = 0; i < CharIntHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(CharIntHashMap.this.keys[i]) && item == CharIntHashMap.this.values[i])
                {
        ...
        public boolean retainAll(IntIterable source)
        {
            ...
            CharIntHashMap retained = CharIntHashMap.this.select((char key, int value) -> sourceSet.contains(value));
            if (retained.size() != oldSize)
            {
                CharIntHashMap.this.keys = retained.keys;
                CharIntHashMap.this.values = retained.values;
    ...
    private class KeyValuesView extends AbstractLazyIterable<CharIntPair>
    {
        @Override
        public void each(Procedure<? super CharIntPair> procedure)
        {
            if (CharIntHashMap.this.sentinelValues != null)
            {
                ...
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, CharIntHashMap.this.sentinelValues.zeroValue));
                }
                if (CharIntHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, CharIntHashMap.this.sentinelValues.oneValue));
                }
            }
            for (int i = 0; i < CharIntHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(CharIntHashMap.this.keys[i]))
                {
        ...
        public void forEachWithIndex(ObjectIntProcedure<? super CharIntPair> objectIntProcedure)
        {
            ...
            if (CharIntHashMap.this.sentinelValues != null)
            {
                if (CharIntHashMap.this.sentinelValues.containsZeroKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(EMPTY_KEY, CharIntHashMap.this.sentinelValues.zeroValue), index);
                    index++;
                }
                if (CharIntHashMap.this.sentinelValues.containsOneKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(REMOVED_KEY, CharIntHashMap.this.sentinelValues.oneValue), index);
                    index++;
                }
            }
            for (int i = 0; i < CharIntHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(CharIntHashMap.this.keys[i]))
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(CharIntHashMap.this.keys[i], CharIntHashMap.this.values[i]), index);
                    index++;
                }
            }
        }
        ...
        public <P> void forEachWith(Procedure2<? super CharIntPair, ? super P> procedure, P parameter)
        {
            if (CharIntHashMap.this.sentinelValues != null)
            {
                if (CharIntHashMap.this.sentinelValues.containsZeroKey)
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, CharIntHashMap.this.sentinelValues.zeroValue), parameter);
                }
                if (CharIntHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, CharIntHashMap.this.sentinelValues.oneValue), parameter);
                }
            }
            for (int i = 0; i < CharIntHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(CharIntHashMap.this.keys[i]))
                {
                    procedure.value(PrimitiveTuples.pair(CharIntHashMap.this.keys[i], CharIntHashMap.this.values[i]), parameter);
                }
            }
        }
        ...
        public class InternalKeyValuesIterator implements Iterator<CharIntPair>
        {
            ...
            public CharIntPair next()
            {
                ...
                if (!this.handledZero)
                {
                    ...
                    if (CharIntHashMap.this.containsKey(EMPTY_KEY))
                    {
                        return PrimitiveTuples.pair(EMPTY_KEY, CharIntHashMap.this.sentinelValues.zeroValue);
                    }
                }
                if (!this.handledOne)
                {
                    ...
                    if (CharIntHashMap.this.containsKey(REMOVED_KEY))
                    {
                        return PrimitiveTuples.pair(REMOVED_KEY, CharIntHashMap.this.sentinelValues.oneValue);
                    }
                }
