...
public class ByteIntHashMap extends AbstractMutableIntValuesMap implements MutableByteIntMap, Externalizable, MutableByteKeysMap
{
    ...
    {
        if (map instanceof ByteIntHashMap && ((ByteIntHashMap) map).occupiedWithSentinels == 0)
        {
            ByteIntHashMap hashMap = (ByteIntHashMap) map;
    ...
    private int getForSentinel(byte key, int ifAbsent)
    {
        if (isEmptyKey(key))
        {
            if (this.sentinelValues == null || !this.sentinelValues.containsZeroKey)
    ...
    private int slowGetIfAbsent(byte key, int ifAbsent)
    {
        int index = this.probe(key);
        if (this.keys[index] == key)
        {
    ...
    {
        MutableIntByteMap result = IntByteMaps.mutable.empty();
    ...
    private class InternalIntIterator implements MutableIntIterator
    {
        ...
        public int next()
        {
            ...
            if (!this.handledZero)
            {
                ...
                if (ByteIntHashMap.this.containsKey(EMPTY_KEY))
                {
                    this.lastKey = EMPTY_KEY;
                    return ByteIntHashMap.this.get(EMPTY_KEY);
                }
            }
            if (!this.handledOne)
            {
                this.handledOne = true;
                if (ByteIntHashMap.this.containsKey(REMOVED_KEY))
                {
                    this.lastKey = REMOVED_KEY;
                    return ByteIntHashMap.this.get(REMOVED_KEY);
                }
            }
    ...
    private class KeySet extends AbstractMutableByteKeySet
    {
        ...
        {
            return ByteIntHashMap.this.keys[index];
        }
        ...
        public boolean retainAll(ByteIterable source)
        {
            ...
            ByteIntHashMap retained = ByteIntHashMap.this.select((byte key, int value) -> sourceSet.contains(key));
            if (retained.size() != oldSize)
            {
                ByteIntHashMap.this.keys = retained.keys;
                ByteIntHashMap.this.values = retained.values;
        ...
        public ByteSet freeze()
        {
            ...
            {
                containsZeroKey = ByteIntHashMap.this.sentinelValues.containsZeroKey;
                containsOneKey = ByteIntHashMap.this.sentinelValues.containsOneKey;
            }
            return new ImmutableByteMapKeySet(ByteIntHashMap.this.keys, ByteIntHashMap.this.occupiedWithData, containsZeroKey, containsOneKey);
        }
    ...
    private class ValuesCollection extends AbstractIntValuesCollection
    {
        ...
        public boolean remove(int item)
        {
            ...
            if (ByteIntHashMap.this.sentinelValues != null)
            {
                ...
                {
                    ByteIntHashMap.this.removeKey(EMPTY_KEY);
                }
                if (ByteIntHashMap.this.sentinelValues.containsOneKey && item == ByteIntHashMap.this.sentinelValues.oneValue)
                {
                    ByteIntHashMap.this.removeKey(REMOVED_KEY);
                }
            }
            for (int i = 0; i < ByteIntHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ByteIntHashMap.this.keys[i]) && item == ByteIntHashMap.this.values[i])
                {
        ...
        public boolean retainAll(IntIterable source)
        {
            ...
            ByteIntHashMap retained = ByteIntHashMap.this.select((byte key, int value) -> sourceSet.contains(value));
            if (retained.size() != oldSize)
            {
                ByteIntHashMap.this.keys = retained.keys;
                ByteIntHashMap.this.values = retained.values;
    ...
    private class KeyValuesView extends AbstractLazyIterable<ByteIntPair>
    {
        @Override
        public void each(Procedure<? super ByteIntPair> procedure)
        {
            if (ByteIntHashMap.this.sentinelValues != null)
            {
                ...
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, ByteIntHashMap.this.sentinelValues.zeroValue));
                }
                if (ByteIntHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, ByteIntHashMap.this.sentinelValues.oneValue));
                }
            }
            for (int i = 0; i < ByteIntHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ByteIntHashMap.this.keys[i]))
                {
        ...
        public void forEachWithIndex(ObjectIntProcedure<? super ByteIntPair> objectIntProcedure)
        {
            ...
            if (ByteIntHashMap.this.sentinelValues != null)
            {
                if (ByteIntHashMap.this.sentinelValues.containsZeroKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(EMPTY_KEY, ByteIntHashMap.this.sentinelValues.zeroValue), index);
                    index++;
                }
                if (ByteIntHashMap.this.sentinelValues.containsOneKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(REMOVED_KEY, ByteIntHashMap.this.sentinelValues.oneValue), index);
                    index++;
                }
            }
            for (int i = 0; i < ByteIntHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ByteIntHashMap.this.keys[i]))
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(ByteIntHashMap.this.keys[i], ByteIntHashMap.this.values[i]), index);
                    index++;
                }
            }
        }
        ...
        public <P> void forEachWith(Procedure2<? super ByteIntPair, ? super P> procedure, P parameter)
        {
            if (ByteIntHashMap.this.sentinelValues != null)
            {
                if (ByteIntHashMap.this.sentinelValues.containsZeroKey)
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, ByteIntHashMap.this.sentinelValues.zeroValue), parameter);
                }
                if (ByteIntHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, ByteIntHashMap.this.sentinelValues.oneValue), parameter);
                }
            }
            for (int i = 0; i < ByteIntHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ByteIntHashMap.this.keys[i]))
                {
                    procedure.value(PrimitiveTuples.pair(ByteIntHashMap.this.keys[i], ByteIntHashMap.this.values[i]), parameter);
                }
            }
        }
        ...
        public class InternalKeyValuesIterator implements Iterator<ByteIntPair>
        {
            ...
            public ByteIntPair next()
            {
                ...
                if (!this.handledZero)
                {
                    ...
                    if (ByteIntHashMap.this.containsKey(EMPTY_KEY))
                    {
                        return PrimitiveTuples.pair(EMPTY_KEY, ByteIntHashMap.this.sentinelValues.zeroValue);
                    }
                }
                if (!this.handledOne)
                {
                    ...
                    if (ByteIntHashMap.this.containsKey(REMOVED_KEY))
                    {
                        return PrimitiveTuples.pair(REMOVED_KEY, ByteIntHashMap.this.sentinelValues.oneValue);
                    }
                }
