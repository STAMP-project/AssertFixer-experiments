...
public class ShortDoubleHashMap extends AbstractMutableDoubleValuesMap implements MutableShortDoubleMap, Externalizable, MutableShortKeysMap
{
    ...
    {
        if (map instanceof ShortDoubleHashMap && ((ShortDoubleHashMap) map).occupiedWithSentinels == 0)
        {
            ShortDoubleHashMap hashMap = (ShortDoubleHashMap) map;
    ...
    private void addKeyValueAtIndex(short key, double value, int index)
    {
    ...
    private double getForSentinel(short key, double ifAbsent)
    {
        if (isEmptyKey(key))
        {
            if (this.sentinelValues == null || !this.sentinelValues.containsZeroKey)
    ...
    private double slowGetIfAbsent(short key, double ifAbsent)
    {
        int index = this.probe(key);
        if (this.keys[index] == key)
        {
    ...
    private double fastGetIfAbsent(short key, double ifAbsent)
    {
    ...
    private double slowGetIfAbsentTwo(short key, double ifAbsent)
    {
    ...
    {
        ShortDoubleHashMap result = new ShortDoubleHashMap();
    ...
    public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException
    {
        ...
        {
            this.put(in.readShort(), in.readDouble());
        }
    }
    ...
    private class InternalDoubleIterator implements MutableDoubleIterator
    {
        ...
        public double next()
        {
            ...
            if (!this.handledZero)
            {
                ...
                if (ShortDoubleHashMap.this.containsKey(EMPTY_KEY))
                {
                    this.lastKey = EMPTY_KEY;
                    return ShortDoubleHashMap.this.get(EMPTY_KEY);
                }
            }
            if (!this.handledOne)
            {
                this.handledOne = true;
                if (ShortDoubleHashMap.this.containsKey(REMOVED_KEY))
                {
                    this.lastKey = REMOVED_KEY;
                    return ShortDoubleHashMap.this.get(REMOVED_KEY);
                }
            }
            short[] keys = ShortDoubleHashMap.this.keys;
            while (!isNonSentinel(keys[this.position]))
            {
                this.position++;
            }
            this.lastKey = keys[this.position];
            double result = ShortDoubleHashMap.this.values[this.position];
            this.position++;
    ...
    private class KeySet extends AbstractMutableShortKeySet
    {
        ...
        {
            return ShortDoubleHashMap.this.keys[index];
        }
        ...
        public boolean retainAll(ShortIterable source)
        {
            ...
            ShortDoubleHashMap retained = ShortDoubleHashMap.this.select((short key, double value) -> sourceSet.contains(key));
            if (retained.size() != oldSize)
            {
                ShortDoubleHashMap.this.keys = retained.keys;
                ShortDoubleHashMap.this.values = retained.values;
                ShortDoubleHashMap.this.sentinelValues = retained.sentinelValues;
                ShortDoubleHashMap.this.occupiedWithData = retained.occupiedWithData;
                ShortDoubleHashMap.this.occupiedWithSentinels = retained.occupiedWithSentinels;
                return true;
            }
            return false;
        }
        ...
        public ShortSet freeze()
        {
            ...
            {
                containsZeroKey = ShortDoubleHashMap.this.sentinelValues.containsZeroKey;
                containsOneKey = ShortDoubleHashMap.this.sentinelValues.containsOneKey;
            }
            return new ImmutableShortMapKeySet(ShortDoubleHashMap.this.keys, ShortDoubleHashMap.this.occupiedWithData, containsZeroKey, containsOneKey);
        }
    ...
    private class ValuesCollection extends AbstractDoubleValuesCollection
    {
        ...
        public boolean remove(double item)
        {
            ...
            if (ShortDoubleHashMap.this.sentinelValues != null)
            {
                if (ShortDoubleHashMap.this.sentinelValues.containsZeroKey && Double.compare(item, ShortDoubleHashMap.this.sentinelValues.zeroValue) == 0)
                {
                    ShortDoubleHashMap.this.removeKey(EMPTY_KEY);
                }
                if (ShortDoubleHashMap.this.sentinelValues.containsOneKey && Double.compare(item, ShortDoubleHashMap.this.sentinelValues.oneValue) == 0)
                {
                    ShortDoubleHashMap.this.removeKey(REMOVED_KEY);
                }
            }
            for (int i = 0; i < ShortDoubleHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ShortDoubleHashMap.this.keys[i]) && Double.compare(item, ShortDoubleHashMap.this.values[i]) == 0)
                {
        ...
        public boolean retainAll(DoubleIterable source)
        {
            ...
            ShortDoubleHashMap retained = ShortDoubleHashMap.this.select((short key, double value) -> sourceSet.contains(value));
            if (retained.size() != oldSize)
            {
                ShortDoubleHashMap.this.keys = retained.keys;
                ShortDoubleHashMap.this.values = retained.values;
    ...
    private class KeyValuesView extends AbstractLazyIterable<ShortDoublePair>
    {
        @Override
        public void each(Procedure<? super ShortDoublePair> procedure)
        {
            if (ShortDoubleHashMap.this.sentinelValues != null)
            {
                ...
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, ShortDoubleHashMap.this.sentinelValues.zeroValue));
                }
                if (ShortDoubleHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, ShortDoubleHashMap.this.sentinelValues.oneValue));
                }
            }
            for (int i = 0; i < ShortDoubleHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ShortDoubleHashMap.this.keys[i]))
                {
        ...
        public void forEachWithIndex(ObjectIntProcedure<? super ShortDoublePair> objectIntProcedure)
        {
            ...
            if (ShortDoubleHashMap.this.sentinelValues != null)
            {
                if (ShortDoubleHashMap.this.sentinelValues.containsZeroKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(EMPTY_KEY, ShortDoubleHashMap.this.sentinelValues.zeroValue), index);
                    index++;
                }
                if (ShortDoubleHashMap.this.sentinelValues.containsOneKey)
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(REMOVED_KEY, ShortDoubleHashMap.this.sentinelValues.oneValue), index);
                    index++;
                }
            }
            for (int i = 0; i < ShortDoubleHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ShortDoubleHashMap.this.keys[i]))
                {
                    objectIntProcedure.value(PrimitiveTuples.pair(ShortDoubleHashMap.this.keys[i], ShortDoubleHashMap.this.values[i]), index);
                    index++;
                }
            }
        }
        ...
        @Override
        public <P> void forEachWith(Procedure2<? super ShortDoublePair, ? super P> procedure, P parameter)
        {
            if (ShortDoubleHashMap.this.sentinelValues != null)
            {
                if (ShortDoubleHashMap.this.sentinelValues.containsZeroKey)
                {
                    procedure.value(PrimitiveTuples.pair(EMPTY_KEY, ShortDoubleHashMap.this.sentinelValues.zeroValue), parameter);
                }
                if (ShortDoubleHashMap.this.sentinelValues.containsOneKey)
                {
                    procedure.value(PrimitiveTuples.pair(REMOVED_KEY, ShortDoubleHashMap.this.sentinelValues.oneValue), parameter);
                }
            }
            for (int i = 0; i < ShortDoubleHashMap.this.keys.length; i++)
            {
                if (isNonSentinel(ShortDoubleHashMap.this.keys[i]))
                {
                    procedure.value(PrimitiveTuples.pair(ShortDoubleHashMap.this.keys[i], ShortDoubleHashMap.this.values[i]), parameter);
                }
            }
        }
        ...
        public class InternalKeyValuesIterator implements Iterator<ShortDoublePair>
        {
            ...
            public ShortDoublePair next()
            {
                ...
                if (!this.handledZero)
                {
                    ...
                    if (ShortDoubleHashMap.this.containsKey(EMPTY_KEY))
                    {
                        return PrimitiveTuples.pair(EMPTY_KEY, ShortDoubleHashMap.this.sentinelValues.zeroValue);
                    }
                }
                if (!this.handledOne)
                {
                    ...
                    if (ShortDoubleHashMap.this.containsKey(REMOVED_KEY))
                    {
                        return PrimitiveTuples.pair(REMOVED_KEY, ShortDoubleHashMap.this.sentinelValues.oneValue);
                    }
                }
                ...
                ShortDoublePair result = PrimitiveTuples.pair(keys[this.position], ShortDoubleHashMap.this.values[this.position]);
                this.position++;
                return result;
            }
