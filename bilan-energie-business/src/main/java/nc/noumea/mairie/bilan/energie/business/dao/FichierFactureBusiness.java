...
public class FichierFactureBusiness extends
    ...
    @Override
    public List<FichierFactureSimple> getAllATraiter() throws TechnicalException, BusinessException {
        List<FichierFactureEntity> listeFichierFactureEntity = (List<FichierFactureEntity>) sessionFactory
                .getCurrentSession()
                .createQuery(
                        "from " + getEntityClass().getName() +
                                " where date_integration_succes is null"
                                + " order by date_integration desc").list();
	...
	@Override
	public List<FichierFactureSimple> getAllSimilaire(FichierFactureSimple fichierFacture) {
        if (fichierFacture.getNom() != null && fichierFacture.getMd5() != null) {
            Query query = sessionFactory
                    .getCurrentSession()
                    .createQuery(
                            "from " + FichierFactureEntity.class.getName()
                                    + " where nom = :nom"
                                    + " and md5 = :md5"
                                    + "	and id !=:id");
            query.setParameter("nom", fichierFacture.getNom());
            query.setParameter("md5", fichierFacture.getMd5());
            query.setParameter("id", fichierFacture.getId());

            return cm.convertList((List<FichierFactureEntity>) query.list(), getDtoClass());
        }
        return null;
	}
