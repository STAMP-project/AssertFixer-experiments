...
public interface OrderedIntIterable extends IntIterable
{
    ...
    @Override
    <V> OrderedIterable<V> collect(IntToObjectFunction<? extends V> function);
     ...
     * @since 9.1.
     */
    default <V> OrderedIterable<V> collectWithIndex(IntIntToObjectFunction<? extends V> function)
    {
        int[] index = { 0 };
     ...
     * @since 9.1.
     */
    default <V, R extends Collection<V>> R collectWithIndex(IntIntToObjectFunction<? extends V> function, R target)
    {
        int[] index = { 0 };
        return this.collect(each -> function.value(each, index[0]++), target);
    }

    <T> T injectIntoWithIndex(T injectedValue, ObjectIntIntToObjectFunction<? super T, ? extends T> function);
