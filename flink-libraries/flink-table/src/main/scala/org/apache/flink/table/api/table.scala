...
import org.apache.flink.table.expressions.{Alias, Asc, Expression, ExpressionParser, Ordering, ProctimeAttribute, RowtimeAttribute, UnresolvedAlias, UnresolvedFieldReference, WindowProperty}
...
class Table(
  ...
  /**
    * Creates table version function backed up by this table as a history table.
    *
    * @param version field for the [[TableVersionFunction]]. Must points to a time indicator
  ...
  def createTableVersionFunction(
    ...
    createTableVersionFunction(
      ExpressionParser.parseExpression(version),
  ...
  def createTableVersionFunction(
      version: Expression,
      primaryKey: Expression): TableVersionFunction = {
    val (versionField, isProctime) = validateVersionExpression(version)
    TableVersionFunction.create(
      this,
      versionField,
      validatePrimaryKeyExpression(primaryKey),
      isProctime)
  }

  private def validateVersionExpression(expression: Expression): (String, Boolean) = {
    expression match {
      case rowtime: RowtimeAttribute =>
        (validateTimeAttribute(rowtime.expression), false)
      case proctime: ProctimeAttribute =>
        (validateTimeAttribute(proctime.expression), true)
      case fieldReference: UnresolvedFieldReference =>
        val fieldName = validateFieldReference(
          getSchema.getType(fieldReference.name),
          fieldReference.name,
          tableEnv.isInstanceOf[StreamTableEnvironment])
        (fieldName, isProctimeIndicatorType(getSchema.getType(fieldName).get))
      case _ => throw new ValidationException(
        s"Version expression [${expression}] must be rowtime or proctime")
    }
  }

  private def validateTimeAttribute(expression: Expression): String = {
    expression match {
      case fieldReference: UnresolvedFieldReference =>
        fieldReference.name
      case _ => throw new ValidationException(
        s"Only field references are supported as version expressions at the moment")
    }
  }
  ...
  private def validateFieldReference(
      fieldReferenceType: Option[TypeInformation[_]],
      name: String,
      mustBeTimeIndicator: Boolean = false): String = {
    if (fieldReferenceType.isEmpty) {
      throw new ValidationException(s"Unknown field [${name}]")
    }
    if (mustBeTimeIndicator && !fieldReferenceType.get.isInstanceOf[TimeIndicatorTypeInfo]) {
      throw new ValidationException(
        s"Version expression [${name}] must point to rowtime or proctime field")
    }
    name
  }
