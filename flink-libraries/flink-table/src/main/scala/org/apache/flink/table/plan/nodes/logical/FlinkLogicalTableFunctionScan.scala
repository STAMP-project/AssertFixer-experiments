...
class FlinkLogicalTableFunctionScanConverter
  ...
  /**
    * This rule do not match to [[TableVersionFunction]]. We do not support reading from
    * [[TableVersionFunction]]s as TableFunctions. We expect them to be rewritten into
    * [[org.apache.flink.table.plan.nodes.datastream.DataStreamScan]] followed by for
    * example [[org.apache.flink.table.plan.nodes.datastream.DataStreamVersionedJoin]].
    */
  override def matches(call: RelOptRuleCall): Boolean = {
    val logicalTableFunction: LogicalTableFunctionScan = call.rel(0)

    !isTableVersionFunctionCall(logicalTableFunction)
  }

  private def isTableVersionFunctionCall(logicalTableFunction: LogicalTableFunctionScan)
    : Boolean = {
    ...
    tableFunction.getTableFunction.isInstanceOf[TableVersionFunction]
  }
