...
import org.apache.flink.table.api.stream.table.VersionedJoinTest
import org.apache.flink.table.api.stream.table.VersionedJoinTest._
...
class VersionedJoinTest extends TableTestBase {
  ...
  val rates = util.addFunction(
    "Rates",
    ratesHistory.createTableVersionFunction('rowtime, 'currency))
  ...
  def testSimpleVersionedJoin(): Unit = {
    ...
    util.verifySql(sqlQuery, getExpectedSimpleVersionedJoinPlan())
  }
  ...
  def testComplexVersionedJoin(): Unit = {
    ...
    util.verifySql(sqlQuery, getExpectedComplexVersionedJoinPlan())
  }

  @Test
  def testUncorrelatedTablVersionedJoin(): Unit = {
